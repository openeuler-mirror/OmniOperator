...
import static nova.hetu.omniruntime.utils.VectorHelper.transformIntToVecType;

public class ContainerVec
        extends FixedWidthVec
{
    ...
    public ContainerVec(VecAllocator allocator, int vectorCount, int positionCount, long[] vectorAddresses, VecType[] vecTypes)
    {
        super(allocator, vectorCount * BYTES, vectorCount, OMNI_VEC_TYPE_CONTAINER);
    ...
    {
        super(vectorCount * BYTES, vectorCount, OMNI_VEC_TYPE_CONTAINER);
        this.positionCount = positionCount;
        this.vecTypes = vecTypes;
        put(vectorAddresses, 0, 0, vectorAddresses.length);
    }
    ...
    public ContainerVec(long nativeVector)
    {
        ...
        this.vecTypes = transformIntToVecType(getVecTypesNative(nativeVector));
    }
    ...
    private ContainerVec(ContainerVec vector, int[] positions, int offset, int length, int positionCount, VecType[] vecTypes)
    {
    ...
    private ContainerVec(ContainerVec vector, int offset, int length, boolean isSlice, int positionCount, VecType[] vecTypes)
    {
    ...
    {
        super(capacityInBytes, data.limit(), OMNI_VEC_TYPE_CONTAINER);
    }
    ...
    private static native int[] getVecTypesNative(long nativeVector);
}
