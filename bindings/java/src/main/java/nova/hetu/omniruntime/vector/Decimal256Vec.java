...
import nova.hetu.omniruntime.type.Decimal256VecType;
 ...
 * @since 2021-07-17
 */
public class Decimal256Vec extends DecimalVec {
    private static final int BYTES = Long.BYTES * 4;

    public Decimal256Vec(int size, int precision, int scale) {
        super(size, BYTES, new Decimal256VecType(precision, scale));
    }

    public Decimal256Vec(VecAllocator allocator, int size, int precision, int scale) {
        super(allocator, size, BYTES, new Decimal256VecType(precision, scale));
    }

    public Decimal256Vec(long nativeVector, VecType type) {
