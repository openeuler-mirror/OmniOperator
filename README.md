...
- \[2025.12.30\]：发布OmniOperator 2.0.0。新增适配Spark的适配层Gluten 1.3版本；SparkExtension新增支持concat\_ws、regexp、regexp\_replace、trim和floor表达式。
- \[2025.06.30\]：发布OmniOperator 1.9.0。Spark 3.3.1新增Parquet格式，支持列式写入；新增支持操作系统CentOS 7.9。
- \[2025.03.30\]：发布OmniOperator 1.8.0。OmniOperator算子加速适配Hive引擎；新增支持Spark 3.4.3、Spark 3.5.2版本。
- \[2024.12.30\]：发布OmniOperator 1.7.0。OmniOperator算子加速适配Hive引擎；新增算子加速支持算子、表达式详情。
- \[2024.09.30\]：发布OmniOperator 1.6.0。新增支持greatest/contains表达式，支持含标量子查询表达式的filter算子不回退；优化TableScan、HashJoin、Shuffle、RollUp算子性能；优化OmniOperator算子加速部署形态，支持Yarn模式下自动部署。
- \[2024.06.30\]：发布OmniOperator 1.5.0。OmniOperator算子加速内存优化，支持大宽表查询；新增支持not表达式、支持Spark inset场景下的AnsiCast表达式；新增对Hive引擎的支持，TPCDS性能提升20%。
- \[2024.03.30\]：发布OmniOperator 1.4.0。新增支持Spark 3.3.1版本（TPC-DS性能提升30%），NullType数据类型，SubqueryBroadcast/Coalesce/Limit算子，instr/startswith/endswith函数，安全集群下ORC和Parquet格式数据处理，Window和HashAggregator算子的Spill功能，HashJoin、Sort和Aggregator算子的NEON指令优化。
- \[2024.10.24\]：发布OmniOperator 1.3.0。Spark 3.1.1版本TPC-DS性能提升30%；新增支持TopNSort、SortMergeJoin和Sort融合算子；新增支持Parquet数据格式处理。
- \[2023.06.30\]：发布OmniOperator 1.2.0。Spark 3.1.1版本支持跑通TPC-DS数据集，性能提升20%。
- \[2022.02.22\]：发布OmniOperator 1.1.0。新增支持Spark 3.1.1版本，成功运行大数据联盟的10条SQL标准测试用例；新增支持UDF功能。
...
- 不同的负载所需内存配置不一样，例如TPC-DS 3TB数据集，SparkExtension推荐配置下，堆外内存配置不低于20GB，99条所有SQL可成功运行，运行过程中日志可能出现MEM\_CAP\_EXCEEDED但不影响最终功能，建议适当增大堆外内存配置。如果堆外内存配置过低，SQL执行结果可能不正确。
- Spark OmniOperator算子加速支持from\_unixtime和unix\_timestamp表达式：
...
- OmniOperator算子加速支持Spark 3.5.2中的ROW\_NUMBER和rank函数计算，面对dense\_rank场景时会算子回退。
...
│   └── en                                                    # 英文文档目录
│       ├── figures                                          # 英文文档图片资源目录
│       ├── public_sys-resources                             # 英文文档图标资源目录
│       ├── faq.md                                           # FAQ (English)
│       ├── installation_guide.md                            # Installation guide (English)
│       ├── release_notes.md                                 # Release notes (English)
...
安装OmniOperator后如何快速验证OmniOperator是否生效，性能是否提升，具体请参见[《快速入门》](./docs/zh/quick_start.md)。
...
|名称|路径|简介|
|--|--|--|
|快速入门|[quick_start.md](./docs/zh/quick_start.md)|提供快速使能并验证OmniOperator加速能力的快速入门指导。|
|版本说明书|[release_notes.md](./docs/zh/release_notes.md)|提供OmniOperator每个发布版本的基础信息和特性更新信息。|
|安装指南|[installation_guide.md](./docs/zh/installation_guide.md)|提供安装OmniOperator的详细指导。|
|使用指南|[user_guide.md](./docs/zh/user_guide.md)|提供使用OmniOperator的详细指导。|
|最佳实践|[best_practices.md](./docs/zh/best_practices.md)|提供OmniOperator的实践案例。|
|常见问题|[faq.md](./docs/zh/faq.md)|提供OmniOperator安装、使用过程的常见问题和解决方法。|
|视频课程|OmniRuntime特性大揭秘如何安装使用OmniOperator算子加速|提供操作视频，帮助开发者在鲲鹏服务器上了解、使能OmniRuntime特性。|
|在线实验|大数据OmniOperator算子加速特性实验|指导开发者使用OmniOperator组件实现大数据计算引擎的算子加速功能。|
|技术干货|大数据技术干货|提供大数据平台在组件部署、应用加速与性能调优方面的实战经验。|
...
## 开源软件声明<a name="ZH-CN_TOPIC_0000002515665490"></a>

下载地址：

[Kunpeng BoostKit 25.3.0 大数据OmniRuntime 开源软件声明.docx](resource/Kunpeng BoostKit 25.3.0 大数据OmniRuntime 开源软件声明.docx)
