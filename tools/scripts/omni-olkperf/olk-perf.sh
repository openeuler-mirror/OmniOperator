...
binpath=/opt/cicd/openlookeng/hetu-server-1.2.0/bin
...
resultdir=/opt/omniruntime/result_olk_$time
 ...
 do
	...
	then
        ...
        then
            ...
            echo "olk--catalog:$catalog--schema:$schema--session:$sessionset" > $cputime_result
        fi
		if [[ ! -f $elapsedtime_result ]]
        then
            touch $elapsedtime_result
            echo "olk--catalog:$catalog--schema:$schema--session:$sessionset" > $elapsedtime_result
        fi
        if [[ $res == 0 ]]
        then
