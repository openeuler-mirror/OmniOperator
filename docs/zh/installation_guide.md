...
|硬件环境|管理/计算/存储节点|
|--|--|
|处理器|鲲鹏920系列处理器<br>鲲鹏950处理器<br>![](./public_sys-resources/icon-caution.gif)**须知**<br>只有支持SVE（Scalable Vector Extension，可扩展向量指令集）指令集的服务器支持在Gluten上使能OmniOperator。您可以通过**cat /proc/cpuinfo \| grep sve \| head -n 1**查询是否支持SVE指令集，如果有输出则表示支持。|
|内存大小|384GB（12 * 32GB）|
|内存频率|2666MHz|
...
|--|----------------------------------------------------------|----------------------------------------------------------------------------------------------------|--|--|
|操作系统| CentOS 7.9<br>openEuler 20.03 LTS SP1<br>openEuler 22.03 LTS SP1<br>openEuler 24.03 LTS SP1 | 例如openEuler 20.03 LTS SP3、openEuler 22.03 LTS SP3、openEuler 24.03 LTS SP3等后续补丁版本亦可。              |√|√|
|JDK| [毕昇JDK 1.8（毕昇JDK 1.8.0_342）](https://mirror.iscas.ac.cn/kunpeng/archive/compiler/bisheng_jdk/bisheng-jdk-8u342-linux-aarch64.tar.gz)<br>[毕昇JDK 17](https://mirrors.huaweicloud.com/kunpeng/archive/compiler/bisheng_jdk/bisheng-jdk-17.0.18-b13-linux-aarch64.tar.gz) | openEuler 22.03 LTS SP1与毕昇JDK 1.8.0_262不兼容，需更换为[毕昇JDK 1.8.0_342](https://mirror.iscas.ac.cn/kunpeng/archive/compiler/bisheng_jdk/bisheng-jdk-8u342-linux-aarch64.tar.gz)。毕昇JDK安装指南请参见《[毕昇JDK 8 安装指南](https://atomgit.com/openeuler/bishengjdk-8/wiki/BiSheng_JDK8_Installation_Guide.md)》。 |√|√|
...
>- SparkExtension支持在CentOS 7.9、openEuler 20.03和openEuler 22.03上运行。
>- Gluten支持在openEuler 22.03和openEuler 24.03上运行。
...
<table border="1" cellpadding="6" cellspacing="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>开源</td>
      <td>OmniOperator算子加速软件安装包（适配Gluten、Spark 3.5.2）。</td>
      <td><a href="https://atomgit.com/openeuler/OmniOperator/releases/download/26.1.0-OmniOperator-2.2.0-release/BoostKit-omniruntime-gluten-2.2.0.zip">获取链接</a></td>
    </tr>
    <tr>      
      <td>Boostkit-omniruntime-gluten-2.0.0.zip</td>
      <td>开源</td>
      <td>OmniOperator算子加速软件安装包（适配Gluten、Spark 3.4.3）。</td>
    ...
    <tr>      
      ...
      <td>开源</td>
      <td>OmniOperator算子加速软件安装包（适配Gluten、Spark 3.3.1）。</td>
      <td><a href="https://gitcode.com/openeuler/OmniOperator/releases/download/25.3.0-OmniOperator-1.0.0-release/BoostKit-omniruntime-gluten-1.0.0.zip">获取链接</a></td>
    </tr>
    <tr>      
...
从鲲鹏社区获取软件安装包后，需要对其进行校验，确保与网站上的原始软件安装包一致。
...
<table border="1" cellpadding="6" cellspacing="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="https://mirrors.tuna.tsinghua.edu.cn/gnu/gcc/gcc-12.3.0/gcc-12.3.0.tar.gz" target="_blank">获取链接</a></td>
    </tr>
    <tr>
      <td>Autoconf</td>
      <td>2.69</td>
...
通过本地方式安装OmniOperator算子加速时，若结合Spark引擎应用时，需在管理节点安装依赖包LLVM和jemalloc。
...
>![](public_sys-resources/icon-note.gif) **说明：** 
>
>- `BoostKit-omniop_2.0.0.zip`可通过`BoostKit-omniruntime_1.9.0.zip`解压获得。`BoostKit-omniop_2.0.0.zip`中有`boostkit-omniop-operator-2.0.0-aarch64-openeuler.tar.gz`和`boostkit-omniop-operator-2.0.0-aarch64-centos.tar.gz`两个包，分别适用于openEuler和CentOS操作系统，下文以openEuler为例进行说明。
