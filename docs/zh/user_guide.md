...
如果选择通过Gluten框架启用OmniOperator，需要用户安装Spark 3.3.1和Gluten 1.3，Spark的安装要求请参见[安装指南](installation_guide.md)的《操作系统和软件要求》。小节将介绍Gluten的安装步骤、安装后的配置方法，以及如何将OmniOperator算子加速特性应用到Spark引擎中。
...
2. 下载Gluten插件包并解压。
    ...
    从[安装指南](installation_guide.md)的《软件安装包获取》获取`Boostkit-omniruntime-gluten-1.0.0.zip`和`Dependency_library_Gluten.zip`，并上传至管理节点的“/opt/omni-operator/”目录下。
...
4. 配置Gluten。
    1. 将`Boostkit-omniruntime-gluten-1.0.0.zip`和`Dependency_library_Gluten.zip`解压到“/opt/omni-operator/lib”。
