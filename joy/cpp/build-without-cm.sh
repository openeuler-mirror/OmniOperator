...
if [ $1 = 'debug' ] && [ $2 = 'low' ];then
    ...
    clang++-12 -g -ggdb -O2 -fPIC -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/linux/"  src/jni/*.cpp  src/operator/*.cpp src/data/*.cpp src/util/*.cpp src/memory_pool/*.cpp src/harden/Hammer.cpp src/harden/HammerConfig.cpp src/harden/HardenOptimizer.cpp `llvm-config-12 --cxxflags --ldflags --libs` -lpthread -ldl -lgtest -lstdc++ -shared -o libjoy.so
elif [ $1 = 'test' ];then
    clang++-12 -g -O2 -dwarf-3 -fPIC -I"$JAVA_HOME/include" -I"$JAVA_HOME/include/linux/"  -I"./tests/" src/jni/api.cpp src/jni/sort_api.cpp  src/operator/*.cpp src/data/*.cpp src/util/*.cpp src/memory_pool/*.cpp src/harden/Hammer.cpp src/harden/HammerConfig.cpp src/harden/HardenOptimizer.cpp tests/hash_groupby_test.cpp ./main.cpp `llvm-config-12 --cxxflags --ldflags --libs` -lpthread -ldl -lgtest -lstdc++ -o main_test
    ./main_test
else echo "No such building type."
fi
echo -e "${GREEN}==================================COMPILE FINISHED=================================="
echo -e "${NC}"
