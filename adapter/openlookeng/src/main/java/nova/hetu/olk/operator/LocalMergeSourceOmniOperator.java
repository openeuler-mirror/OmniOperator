 ...
 * @since 20210630
 */
public class LocalMergeSourceOmniOperator implements Operator {
    /**
     * The type Local merge source omni operator factory.
     *
     * @since 20210630
     */
    public static class LocalMergeSourceOmniOperatorFactory implements OperatorFactory {
    ...
    public Page getOutput() {
        if (!isFinished && isSourceFinished()) {
            ImmutableList<List<Page>> pageProducers = sources.stream()
                .map(LocalExchangeSource::getPages)
                .collect(toImmutableList());
