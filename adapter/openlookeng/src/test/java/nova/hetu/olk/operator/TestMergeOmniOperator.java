...
public class TestMergeOmniOperator {
    ...
    public void setUp() {
        ...
        exchangeClientConfig.setExchangeEnabled(false);
        exchangeClientFactory = new ExchangeClientFactory(exchangeClientConfig, new ShuffleServiceConfig(), httpClient,
                executor);
    ...
    public void testSingleStream() throws Exception {
        ...
        MergeOmniOperator operator = createMergeOmniOperator(types, ImmutableList.of(1), ImmutableList.of(0, 1),
    ...
    public void testMergeDifferentTypes() throws Exception {
        ...
        MergeOmniOperator operator = createMergeOmniOperator(types, ImmutableList.of(1, 0), ImmutableList.of(1, 0),
                ImmutableList.of(DESC_NULLS_FIRST, ASC_NULLS_FIRST));
        operator.addSplit(createRemoteSplit(TASK_1_ID));
    ...
    public void testMultipleStreamsSameOutputColumns() throws Exception {
        ...
        MergeOmniOperator operator = createMergeOmniOperator(types, ImmutableList.of(0, 1, 2), ImmutableList.of(0),
        ...
        List<Page> source1Pages = rowPagesBuilder(types).row(1, 1, 2).row(8, 1, 1).row(19, 1, 3).row(27, 1, 4)
                .row(41, 2, 5).pageBreak().row(55, 1, 2).row(89, 1, 3).row(101, 1, 4).row(202, 1, 3).row(399, 2, 2)
                .pageBreak().row(400, 1, 1).row(401, 1, 7).row(402, 1, 6).build();

        List<Page> source2Pages = rowPagesBuilder(types).row(2, 1, 2).row(8, 1, 1).row(19, 1, 3).row(25, 1, 4)
                .row(26, 2, 5).pageBreak().row(56, 1, 2).row(66, 1, 3).row(77, 1, 4).row(88, 1, 3).row(99, 2, 2)
                .pageBreak().row(99, 1, 1).row(100, 1, 7).row(100, 1, 6).build();

        List<Page> source3Pages = rowPagesBuilder(types).row(88, 1, 3).row(89, 1, 3).row(90, 1, 3).row(91, 1, 4)
                .row(92, 2, 5).pageBreak().row(93, 1, 2).row(94, 1, 3).row(95, 1, 4).row(97, 1, 3).row(98, 2, 2)
                .build();
    ...
    private MergeOmniOperator createMergeOmniOperator(List<Type> sourceTypes, List<Integer> outputChannels,
            List<Integer> sortChannels, List<SortOrder> sortOrder) {
        int mergeOperatorId = operatorId.getAndIncrement();
        MergeOmniOperator.MergeOmniOperatorFactory factory = new MergeOmniOperator.MergeOmniOperatorFactory(
                mergeOperatorId, mergeOperatorId, new PlanNodeId("plan_node_id" + mergeOperatorId),
                exchangeClientFactory, serdeFactory, orderingCompiler, sourceTypes, outputChannels, sortChannels,
                sortOrder);
        ...
        return (MergeOmniOperator) factory.createOperator(driverContext);
    }

    private static Split createRemoteSplit(String taskId) {
        return new Split(ExchangeOperator.REMOTE_CONNECTOR_ID,
                new RemoteSplit(URI.create("http://localhost/" + taskId)), Lifespan.taskWide());
    }
