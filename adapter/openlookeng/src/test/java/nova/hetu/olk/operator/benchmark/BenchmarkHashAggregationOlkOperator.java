...
public class BenchmarkHashAggregationOlkOperator
{
    ...
    public static class Context
    {
        ...
        public void setup()
        {
            ...
            RowPagesBuilder pagesBuilder = RowPagesBuilder.rowPagesBuilder(hashAggregation, ImmutableList.of(0,1,2), VARCHAR,VARCHAR,VARCHAR, BIGINT, BIGINT, BIGINT);
...
//                    BIGINT.writeLong(blockBuilder, j % rowsPerGroup);
//                }
                pagesBuilder.addBlocksPage(blockBuilder.build(),blockBuilder.build(),blockBuilder.build(),
                        createLongSequenceBlock(0, ROWS_PER_PAGE),createLongSequenceBlock(0, ROWS_PER_PAGE), createLongSequenceBlock(0, ROWS_PER_PAGE));
            }
...
//                    ImmutableList.of(BIGINT),
                    ...
                    false,
                    ImmutableList.of(//COUNT.bind(ImmutableList.of(0), Optional.empty()),
                            LONG_SUM.bind(ImmutableList.of(3), Optional.empty()),LONG_SUM.bind(ImmutableList.of(4), Optional.empty()), LONG_SUM.bind(ImmutableList.of(5), Optional.empty())),
                    hashChannel,
                    Optional.empty(),
                    100_000,
    ...
    private void verify(int rowsPerGroup, String operatorType)
    {
        ...
        assertEquals(Context.TOTAL_PAGES, context.getPages().size());
        for (int i = 0; i < Context.TOTAL_PAGES; i++) {
            assertEquals(Context.ROWS_PER_PAGE, context.getPages().get(i).getPositionCount());
        }

        List<Page> outputPages = benchmark(context);
        assertEquals(Context.TOTAL_PAGES * Context.ROWS_PER_PAGE / rowsPerGroup, outputPages.stream().mapToInt(Page::getPositionCount).sum());
            ...
            throws RunnerException
    {
        Options options = new OptionsBuilder()
                ...
                .include(".*" + BenchmarkHashAggregationOlkOperator.class.getSimpleName() + ".*")
                .build();
