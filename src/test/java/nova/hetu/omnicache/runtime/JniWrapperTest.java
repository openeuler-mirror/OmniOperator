...
public class JniWrapperTest
{
    ...
    public void testOrderByOneColumn()
    {
        ...
        ByteBuffer nullBuffer = nullVec.getData();
        long addr = ((DirectBuffer)inputs[0]).address();
        long[] inputAddr = {addr};
    ...
    public void testOrderByPerformance()
    {
        ...
        long sortAddress = wrapper.allocAndInitSort(sourceTypes, 2, outputCols, 2, sortCols, ascendings, nullFirsts, 1);
        ...
        for (int i = 0; i < vecs.length; i++) {
            ...
            inputs[0] = vecs[i][0].getData();
            inputs[1] = vecs[i][1].getData();
            long addr1 = ((DirectBuffer)inputs[0]).address();
            long addr2 = ((DirectBuffer)inputs[1]).address();
            long[] inputAddr = {addr1, addr2};

            //int[][] nulls = new int[2][rowNum];
            IntVec nullVec = new IntVec(2 * rowNum);
            ByteBuffer nullbuffer = nullVec.getData();

            long instart = System.currentTimeMillis();
            wrapper.addTable(sortAddress, inputAddr, ((DirectBuffer)nullbuffer).address(), 2, rowNum);
