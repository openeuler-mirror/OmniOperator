...
pub unsafe fn transform_input_data<T>(v_ref: &Vec<T>, addr: *mut c_void, offset: isize) {
    ...
    libc::memcpy(
        addr.offset(offset * *WELD_VEC_WIDTH as isize),
        &weld_vec as *const _ as *const c_void,
        *WELD_VEC_WIDTH,
    );
    mem::forget(weld_vec);
}
...
pub fn into_vec<T>(data: *const T, len: i64) -> Vec<T> {
    unsafe { Vec::from_raw_parts(data as *mut T, len as usize, len as usize) }
}
