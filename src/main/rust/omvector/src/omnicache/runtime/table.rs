...
macro_rules! make_column {
    ($name: ident, $ty: ty) => {
        ...
        impl ColumnBuilder {
            pub fn $name(name: &str, data: ManuallyDrop<Vec<$ty>>) -> $name {
                $name { name, data }
            }
        }
    };
...
#[cfg(test)]
mod tests {
    use crate::omnicache::runtime::table::{ColumnBuilder, Table};
    use std::mem::ManuallyDrop;

    #[test]
    fn test_table() {
        let c1 = ColumnBuilder::ColumnI16("c1", ManuallyDrop::new(vec![0i16; 3]));
        let c2 = ColumnBuilder::ColumnI32("c1", ManuallyDrop::new(vec![0i32; 3]));
        let c3 = ColumnBuilder::ColumnF64("c1", ManuallyDrop::new(vec![0f64; 3]));
