...
###############################################################################
## omnitools - test and run automation
##
...
OMNITOOLS_DIR="$(echo "$SCRIPT_PATH" | sed -e "s/\\(.*\\/\\)[^\\/]*$/\\1/")"
OMNITOOLS_DIR="$(cd "$OMNITOOLS_DIR" && pwd)/"
SRC_DIR="${OMNITOOLS_DIR}cmds/"
...
if [ ! "${PROJECT_ROOT+x}" ]; then
  ...
  while true; do
    ...
    if [ ! -f "${PROJECT_ROOT}.omnitools/config.sh" ]; then
...
. "${OMNITOOLS_DIR}misc/help.sh"  # help() function
...
if [ -f "${PROJECT_ROOT}.omnitools/config.sh" ]; then
  ...
  . "${PROJECT_ROOT}.omnitools/config.sh"
else
...
if ! echo "$cmd" | grep -q "^[ ]*docker"; then
  if [ "$IGNORE_IN_DOCKER" = "False" ] && \
    ...
    (. "${SRC_DIR}docker/run.sh"; IN_DOCKER=True FLAG_cmd="omnitools $*" FLAG_tag="$OMNITOOLS_IMAGE" main "$*")
    exit 0
  fi
fi
cmdpath=""
if [ -f "${PROJECT_ROOT}.omnitools/cmds/$(echo "$cmd" | tr ' ' '/').sh" ]; then
  _debug "loading custom command $cmd"
  cmdpath="${PROJECT_ROOT}.omnitools/cmds/$(echo "$cmd" | tr ' ' '/').sh"
