...
main() {
  ...
  # c++
  _subheading "checking for clang-tidy"
  cfiles="$(find "$searchpath" -type f -iname "*.h" -o -iname "*.cpp")"
  if [ "$cfiles" = "" ]; then
    _info "no cfiles found. skipping"
  else
    if command -v "clang-tidy" > /dev/null; then
      _clangtidy "$searchpath" || fail=1
    else
      _warn "c++ files found but clang-tidy not installed. skipping"
    fi
  fi
...
_clangtidy() {
  ...
  find "$1" -iname "*.h" -o -iname "*.cpp" | xargs  clang-tidy
}
