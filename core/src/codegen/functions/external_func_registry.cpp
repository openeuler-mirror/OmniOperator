...
FunctionSignature* getExternalSignature(string funcName) {
    if (funcName == add1_int32_str) {
        ...
        void* fnAddr = (void *) (add1_int32);
        FunctionSignature* add1_int32_sig = new FunctionSignature(add1_int32_str, add1_int32_types, retType, fnAddr);
        return add1_int32_sig;
    }
    else {
        // Vector containing parameter types
        ...
        DataType retType = DataType::INT32D;
        // Void pointer to function address
        void* fnAddr = (void *) (id_int32);
        FunctionSignature* id_int32_sig = new FunctionSignature(id_int32_str, id_int32_types, retType, fnAddr);
        return id_int32_sig;
    }
}