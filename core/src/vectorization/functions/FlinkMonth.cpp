...
public:
    explicit FlinkMonthFunction(bool hasTz) : hasTz_(hasTz) {}
...
{
    auto func = std::make_shared<FlinkMonthFunction>(false);
...
void RegisterFlinkMonthWithTzFunction(const std::string &name)
{
    ...
    auto func = std::make_shared<FlinkMonthFunction>(true);
