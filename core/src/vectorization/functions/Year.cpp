        ...
        op::ExecutionContext *context) const override
    {
        ...
        if (inputTypeId == OMNI_DATE32 || inputTypeId == OMNI_INT) {
            ...
            rows.applyToSelected([&](vector_size_t i) {
                ...
                // Extract year using Timestamp::epochToCalendarUtc (static method)
                std::tm tmValue;
                if (Timestamp::epochToCalendarUtc(seconds, tmValue)) {
                    // tm_year is years since 1900, so we need to add 1900 to get actual year
...
{
    // Year takes DATE32 or INT and returns INT (year, e.g., 2024)
