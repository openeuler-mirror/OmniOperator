...
public:
    ...
    nonexistent_local_time(const date::local_time<_Duration> &__time, const local_info &__info)
        : runtime_error{__create_message(__time, __info)}
    {
        ...
        if (__info.result != local_info::nonexistent) {
            throw std::runtime_error("creating an nonexistent_local_time from a local_info that is not non-existent");
        }
    }
...
private:
    template <class _Duration>
    std::string __create_message(const date::local_time<_Duration> &__time, const local_info &__info)
    {
        using namespace date;

        std::ostringstream os;
        os << __time << " is in a gap between\n" << date::local_seconds{__info.first.end.time_since_epoch()} + __info.
            first.offset << ' ' << __info.first.abbrev << " and\n" << date::local_seconds{
                __info.second.begin.time_since_epoch()
            } + __info.second.offset << ' ' << __info.second.abbrev << " which are both equivalent to\n";

        date::operator<<(os, __info.first.end) << " UTC";
        return os.str();
    }
};

template <class _Duration>
[[noreturn]] void __throw_nonexistent_local_time([[maybe_unused]] const date::local_time<_Duration> &__time,
...
public:
    template <class _Duration>
    ambiguous_local_time(const date::local_time<_Duration> &__time, const local_info &__info)
        : runtime_error{__create_message(__time, __info)}
    {
        ...
        if (__info.result != local_info::ambiguous) {
            throw std::runtime_error("creating an ambiguous_local_time from a local_info that is not ambiguous");
        }
    }
...
private:
    ...
    std::string __create_message(const date::local_time<_Duration> &__time, const local_info &__info)
    {
        std::ostringstream os;
        using date::operator<<;
        os << __time << " is ambiguous.  It could be\n" << __time << ' ' << __info.first.abbrev << " == " << __time -
            __info.first.offset << " UTC or\n" << __time << ' ' << __info.second.abbrev << " == " << __time - __info.
            second.offset << " UTC";
        return os.str();
    }
};
...
template <class _Duration>
[[noreturn]] void __throw_ambiguous_local_time([[maybe_unused]] const date::local_time<_Duration> &__time,
    [[maybe_unused]] const local_info &__info)
{
...
public:
    invalid_time_zone(const std::string_view &__tz_name)
...
private:
    std::string __create_message(const std::string_view &__tz_name)
    {
        std::ostringstream os;
...
[[noreturn]] void __throw_invalid_time_zone([[maybe_unused]] const std::string_view &__tz_name);
}
