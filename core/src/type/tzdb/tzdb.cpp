...
std::string __libcpp_tzdb_directory()
{
    ...
    if (stat(tz_dir_buildroot, &sb) == 0 && S_ISDIR(sb.st_mode)) return tz_dir_buildroot;
    else if (stat(tz_dir_default, &sb) == 0 && S_ISDIR(sb.st_mode)) return tz_dir_default;
    else throw runtime_error("discover_tz_dir failed to find zoneinfo\n");
#else // __APPLE__
...
{
    while (__is_whitespace(__input.peek())) __input.get();
}

static void __skip_mandatory_whitespace(std::istream &__input)
{
    if (!__is_whitespace(__input.get())) std::__throw_runtime_error("corrupt tzdb: expected whitespace");
...
{
    if (std::tolower(__input.peek()) == __suffix) __input.get();
}

static void __skip(std::istream &__input, std::string_view __suffix)
{
    for (auto __c : __suffix)
        if (std::tolower(__input.peek()) == __c) __input.get();
}
...
static void __matches(std::istream &__input, char __expected)
{
    ...
    if (std::tolower(__c) != __expected)
        std::__throw_runtime_error(
            (std::string("corrupt tzdb: expected character '") + __expected + "', got '" + __c + "' instead").c_str());
}
...
static void __matches(std::istream &__input, std::string_view __expected)
{
    for (auto __c : __expected) {
        ...
        if (std::tolower(__actual) != __c)
            std::__throw_runtime_error(
            (std::string("corrupt tzdb: expected character '") + __c + "' from string '" + std::string(__expected) +
                "', got '" + __actual + "' instead").c_str());
    }
}
...
[[nodiscard]] static int64_t __parse_integral(std::istream &__input, bool __leading_zero_allowed)
{
    ...
    if (__leading_zero_allowed) {
        if (__result < '0' || __result > '9') std::__throw_runtime_error("corrupt tzdb: expected a digit");
    } else {
    ...
    while ((c = __input.peek()) >= '0' && c <= '9') {
        ...
        if (__result > (std::numeric_limits<int64_t>::max() / 16)) std::__throw_runtime_error(
            "corrupt tzdb: integral too large");
...
[[nodiscard]] static date::day __parse_day(std::istream &__input)
{
    ...
    if (__result > 31) std::__throw_runtime_error("corrupt tzdb day: value too large");
...
[[nodiscard]] static date::year __parse_year_value(std::istream &__input)
{
    ...
    if (__negative) [[unlikely]] __input.get();
...
{
    if (std::tolower(__input.peek()) != 'm') [[likely]] return __parse_year_value(__input);
...
[[nodiscard]] static date::year __parse_to(std::istream &__input, date::year __only)
{
    if (std::tolower(__input.peek()) != 'o') return __parse_year(__input);
...
[[nodiscard]] static __constrained_weekday::__comparison_t __parse_comparison(std::istream &__input)
{
    switch (__input.get()) {
        case '>':
            __matches(__input, '=');
...
[[nodiscard]] static omniruntime::tzdb::__on __parse_on(std::istream &__input)
{
    if (std::isdigit(__input.peek())) return __parse_day(__input);
    ...
    return __constrained_weekday{__parse_weekday(__input), __parse_comparison(__input), __parse_day(__input)};
}

[[nodiscard]] static std::chrono::seconds __parse_duration(std::istream &__input)
{
    ...
    if (__negative) {
        ...
        if (!std::isdigit(__input.peek())) return __result;
    }

    __result += std::chrono::hours(__parse_integral(__input, true));
    if (__input.peek() != ':') return __negative ? -__result : __result;
...
[[nodiscard]] static omniruntime::tzdb::__clock __parse_clock(std::istream &__input)
{
    switch (__input.get()) {
        // case sensitive
        case 'w':
            return omniruntime::tzdb::__clock::__local;
        case 's':
            return omniruntime::tzdb::__clock::__standard;

        case 'u':
        case 'g':
        case 'z':
            return omniruntime::tzdb::__clock::__universal;
    }
...
[[nodiscard]] static bool __parse_dst(std::istream &__input, std::chrono::seconds __offset)
{
    switch (__input.get()) {
        // case sensitive
        case 's':
...
[[nodiscard]] static omniruntime::tzdb::__at __parse_at(std::istream &__input)
{
    return {__parse_duration(__input), __parse_clock(__input)};
}

[[nodiscard]] static omniruntime::tzdb::__save __parse_save(std::istream &__input)
{
...
[[nodiscard]] static __continuation::__rules_t __parse_rules(std::istream &__input)
{
    int __c = __input.peek();
    // A single -  is not a SAVE but a special case.
    if (__c == '-') {
        __input.get();
        if (__is_whitespace(__input.peek())) return std::monostate{};
        __input.unget();
        return __parse_save(__input);
    }

    if (std::isdigit(__c) || __c == '+') return __parse_save(__input);
...
[[nodiscard]] static __continuation __parse_continuation(__rules_storage_type &__rules,
    std::istream &__input)
{
    __continuation __result;
...
[[nodiscard]] static __rule &__create_entry(__rules_storage_type &__rules, const std::string &__name)
{
    auto __result = [&]() -> __rule & {
        auto rule = __rules.emplace(std::upper_bound(__rules.begin(), __rules.end(), __name,
    ...
    if (__rules.empty()) return __result();

    // Typically rules are in contiguous order in the database.
    // But there are exceptions, some rules are interleaved.
    if (__rules.back().first == __name) return __rules.back().second.emplace_back();

    if (auto __it = std::find_if(__rules.begin(), __rules.end(),
        [&__name](const auto &__r) { return __r.first == __name; }); __it != __rules.end()) return __it->second.
        emplace_back();

    return __result();
}

static void __parse_rule(tzdb &__tzdb, __rules_storage_type &__rules, std::istream &__input)
{
    __skip_mandatory_whitespace(__input);
...
static void __parse_zone(tzdb &__tzdb, __rules_storage_type &__rules, std::istream &__input)
{
    __skip_mandatory_whitespace(__input);
    auto __p = std::make_unique<time_zone::__impl>(__parse_string(__input), __rules);
    std::vector<omniruntime::tzdb::__continuation> &__continuations = __p->__continuations();
    __skip_mandatory_whitespace(__input);
...
static void __parse_tzdata(tzdb &__db, __rules_storage_type &__rules, std::istream &__input)
{
    while (true) {
...
static void __parse_leap_seconds(std::vector<leap_second> &__leap_seconds, std::istream &&__input)
{
    // The file stores dates since 1 January 1900, 00:00:00, we want
    // seconds since 1 January 1970.
    constexpr auto __offset = date::sys_days{date::year(1970) / date::January / 1} - date::sys_days{
        date::year(1900) / date::January / 1
    };
    ...
    [&] {
        while (__input.peek() != std::istream::traits_type::eof()) {
            ...
            date::sys_seconds __date = date::sys_seconds{
                std::chrono::seconds{__parse_integral(__input, false)}
            } - __offset;
            __skip_mandatory_whitespace(__input);
    ...
    std::sort(__entries.begin(), __entries.end(), [](const auto &left_entry, const auto &right_entry) {
    ...
    (void) std::adjacent_find(__entries.begin(), __entries.end(), [&](const __entry &__first, const __entry &__second) {
        __leap_seconds.emplace_back(__second.__timestamp, __second.__value - __first.__value);
        return false;
    });
}
...
void __init_tzdb(tzdb &__tzdb, __rules_storage_type &__rules)
{
    ...
    // opinion.
    __parse_leap_seconds(__tzdb.leap_seconds, std::ifstream{__root / "leap-seconds.list"});
}
...
[[nodiscard]] static const time_zone *__current_zone_posix(const tzdb &tzdb)
{
    ...
    if (const char *__tz = getenv("TZ"))
        if (const time_zone *__result = tzdb.__locate_zone(__tz)) return __result;
    ...
    if (__tz.is_relative()) __tz = std::filesystem::canonical("/etc" / __tz);

    std::string __name = std::filesystem::relative(__tz, "/usr/share/zoneinfo/");
    if (const time_zone *__result = tzdb.__locate_zone(__name)) return __result;
