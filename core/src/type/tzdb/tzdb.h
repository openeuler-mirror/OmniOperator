...
struct tzdb {
    ...
    {
        if (const time_zone *__result = __find_in_zone(__name)) return __result;

        if (auto __it = std::lower_bound(links.begin(), links.end(), __name,
            [](const time_zone_link &link, const std::string_view &name) {
                return link.name() < name;
            }); __it != links.end() && __it->name() == __name)
            if (const time_zone *__result = __find_in_zone(__it->target())) return __result;
...
private:
    ...
    {
        if (auto __it = std::lower_bound(zones.begin(), zones.end(), __name,
