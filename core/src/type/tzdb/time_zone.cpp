...
#include "date.h"
#include "type/tz/tz.h"
#include "time_zone_private.h"
...
[[nodiscard]] static auto __binary_find(const __rules_storage_type &__rules_db, const std::string &__rule_name)
{
    auto __end = __rules_db.end();
    auto __ret = std::lower_bound(__rules_db.begin(), __rules_db.end(), __rule_name,
        [](const auto &rule, const auto &name) { return rule.first < name; });
    if (__ret == __end) return __end;
...
// abbreviation AT.
[[nodiscard]] static std::string __format(const omniruntime::tzdb::__continuation &__continuation,
    const std::string &__letters, std::chrono::seconds __save)
{
    bool __shift = false;
    std::string __result;
    for (char __c : __continuation.__format) {
        if (__shift) {
            switch (__c) {
                case 's':
                    std::copy(__letters.begin(), __letters.end(), std::back_inserter(__result));
                    break;
                ...
                case 'z': {
                    ...
                    if (__offset.minutes() != 0min)
                        fmt::format_to(std::back_inserter(__result), "{:%H}{:%M}", __offset.hours(),
                            __offset.minutes());
                    else
                        fmt::format_to(std::back_inserter(__result), "{:%H}", __offset.hours());
                }
                break;

                default:
                    std::__throw_runtime_error(
                        fmt::format("corrupt tzdb FORMAT field: invalid sequence '%{}' found, expected %s or %z",
                            __c).c_str());
            }
            __shift = false;
        } else if (__c == '/') {
            if (__save != 0s) __result.clear();
            else break;
        } else if (__c == '%') {
            __shift = true;
    ...
    if (__result.empty()) std::__throw_runtime_error("corrupt tzdb FORMAT field: result is empty");

    return __result;
}

[[nodiscard]] static date::sys_seconds __to_sys_seconds(date::year_month_day __ymd, std::chrono::seconds __seconds)
{
    std::chrono::seconds __result = static_cast<date::sys_days>(__ymd).time_since_epoch() + __seconds;
...
[[nodiscard]] static std::chrono::seconds __at_to_sys_seconds(const omniruntime::tzdb::__continuation &__continuation)
{
    switch (__continuation.__at.__clock) {
        case omniruntime::tzdb::__clock::__local:
            return __continuation.__at.__time - __continuation.__stdoff - std::visit([](const auto &__value) {
                using _Tp = std::decay_t<decltype(__value)>;
                if constexpr (std::is_same_v<_Tp, std::monostate>) return std::chrono::seconds{0};
                else if constexpr (std::is_same_v<_Tp, omniruntime::tzdb::__save>)
                    return std::chrono::duration_cast<std::chrono::seconds>(__value.__time);
                ...
                else static_assert(False<_Tp>::value);

                throw std::runtime_error("unreachable");
            }, __continuation.__rules);

        case omniruntime::tzdb::__clock::__universal:
            return __continuation.__at.__time;

        case omniruntime::tzdb::__clock::__standard:
            return __continuation.__at.__time - __continuation.__stdoff;
    }
    throw std::runtime_error("unreachable");
}

[[nodiscard]] static date::year_month_day __to_year_month_day(date::year __year, date::month __month,
    omniruntime::tzdb::__on __on)
{
    return std::visit([&](const auto &__value) {
        using _Tp = std::decay_t<decltype(__value)>;
        if constexpr (std::is_same_v<_Tp, date::day>) return date::year_month_day{__year, __month, __value};
        else if constexpr (std::is_same_v<_Tp, date::weekday_last>)
            return date::year_month_day{
                static_cast<date::sys_days>(date::year_month_weekday_last{__year, __month, __value})
            };
        else if constexpr (std::is_same_v<_Tp, __constrained_weekday>) return __value(__year, __month);
        else static_assert(False<_Tp>::value);
...
[[nodiscard]] static date::sys_seconds __until_to_sys_seconds(const omniruntime::tzdb::__continuation &__continuation)
{
    // Does UNTIL contain the magic value for the last continuation?
    if (__continuation.__year == date::year::min()) return date::sys_seconds::max();

    date::year_month_day __ymd = __to_year_month_day(__continuation.__year, __continuation.__in, __continuation.__on);
...
public:
    explicit __named_rule_until(const omniruntime::tzdb::__continuation &__continuation)
        : __until_{__until_to_sys_seconds(__continuation)}, __needs_adjustment_{
            ...
            __until_ != date::sys_seconds::max() && __continuation.__at.__clock == omniruntime::tzdb::__clock::__local
        } {}
...
[[nodiscard]] static std::chrono::seconds __at_to_seconds(std::chrono::seconds __stdoff, const __rule &__rule)
{
...
[[nodiscard]] static date::sys_seconds __from_to_sys_seconds(std::chrono::seconds __stdoff, const __rule &__rule,
    date::year __year)
{
    date::year_month_day __ymd = __to_year_month_day(__year, __rule.__in, __rule.__on);
...
[[nodiscard]] static const std::vector<__rule> &__get_rules(const __rules_storage_type &__rules_db,
...
[[nodiscard]] static std::string __letters_before_first_rule(const std::vector<__rule> &__rules)
{
...
[[nodiscard]] static sys_info __get_sys_info_before_first_rule(date::sys_seconds __begin, date::sys_seconds __end,
    const omniruntime::tzdb::__continuation &__continuation, const std::vector<__rule> &__rules)
{
    return sys_info{
        __begin,
        __end,
...
[[nodiscard]] static sys_info __get_sys_info_before_first_rule(date::sys_seconds __begin, date::sys_seconds __rule_end,
    // The end used when SAVE != 0s
    date::sys_seconds __next_end, // The end used when SAVE == 0s the times are merged
    const omniruntime::tzdb::__continuation &__continuation, const std::vector<__rule> &__rules,
...
[[nodiscard]] static std::chrono::seconds __at_to_seconds(std::chrono::seconds __stdoff, std::chrono::seconds __save,
    const __rule &__rule)
{
    switch (__rule.__at.__clock) {
        case omniruntime::tzdb::__clock::__local:
...
[[nodiscard]] static date::sys_seconds __rule_to_sys_seconds(std::chrono::seconds __stdoff, std::chrono::seconds __save,
    const __rule &__rule, date::year __year)
{
    date::year_month_day __ymd = __to_year_month_day(__year, __rule.__in, __rule.__on);
...
[[nodiscard]] static std::pair<date::sys_seconds, std::vector<__rule>::const_iterator>
__next_rule(date::sys_seconds __time, std::chrono::seconds __stdoff, std::chrono::seconds __save,
    const std::vector<__rule> &__rules, std::vector<__rule>::const_iterator __current)
{
    date::year __year = date::year_month_day{std::chrono::floor<date::days>(__time)}.year();
    ...
    for (auto __it = __rules.begin(); __it != __rules.end(); ++__it) {
        for (date::year __y = std::max(__year - date::years(1), __it->__from); __y <= __it->__to; ++__y) {
            ...
            if (__y == __year && __it == __current) continue;
    ...
    if (!__candidates.empty()) [[likely]] {
        ...
        if (__time == date::sys_seconds::min()) return *__it;
        ...
        while (__it != __candidates.end()) {
            if (__current->__save.__time != __it->second->__save.__time || __current->__letters != __it->second->
                __letters) return *__it;
...
[[nodiscard]] static std::vector<__rule>::const_iterator __first_rule(std::chrono::seconds __stdoff,
    const std::vector<__rule> &__rules)
{
    return __next_rule(date::sys_seconds::min(), __stdoff, 0s, __rules, __rules.end()).second;
}

[[nodiscard]] static __sys_info_result __get_sys_info_rule(date::sys_seconds __time,
    ...
    const std::vector<__rule> &__rules)
{
    ...
    date::sys_seconds __rule_begin = __from_to_sys_seconds(__continuation.__stdoff, *__rule);
    ...
    auto __next = __next_rule(__rule_begin, __continuation.__stdoff, __rule->__save.__time, __rules, __rule);
    ...
    if (date::sys_seconds __begin = __rule->__save.__time != 0s ? __rule_begin : __next.first; __time < __begin) {
        if (__continuation_begin == date::sys_seconds::min() || __begin - __continuation_begin > 12h)
            return __sys_info{
                __get_sys_info_before_first_rule(__continuation_begin, __rule_begin, __next.first, __continuation,
                    __rules, __rule),
                false
            };
        ...
        date::sys_seconds __sys_info_end = std::min(__continuation_end(__save), __next.first);
    ...
    date::year year = date::year_month_day{std::chrono::floor<date::days>(__time)}.year();
    ...
    if (__continuation.__year == date::year::min() && year >= date::year(1900)) {
        ...
        if (isInForeverRules && numForeverRules == 2) {
            ...
            date::year transitionYear = date::year_month_day{std::chrono::floor<date::days>(__time)}.year() -
                date::years(1);
            date::sys_seconds firstTransition = __rule_to_sys_seconds(__continuation.__stdoff,
                std::get<1>(foreverRules)->__save.__time, *std::get<0>(foreverRules), transitionYear);
            date::sys_seconds secondTransition = __rule_to_sys_seconds(__continuation.__stdoff,
                std::get<0>(foreverRules)->__save.__time, *std::get<1>(foreverRules), transitionYear);
    ...
    while (__next.second != __rules.end()) {
        ...
        date::sys_seconds __sys_info_begin = std::max(__continuation_begin, __rule_begin);
        date::sys_seconds __sys_info_end = std::min(__end, __next.first);
        std::chrono::seconds __diff = std::chrono::abs(__sys_info_end - __sys_info_begin);
        ...
        if ((__time >= __rule_begin && __time < __next.first) || __next.first >= __end) {
            ...
            return __sys_info{
                sys_info{
                    ...
                    std::chrono::duration_cast<std::chrono::minutes>(__rule->__save.__time),
...
[[nodiscard]] static __sys_info_result __get_sys_info_basic(date::sys_seconds __time,
    date::sys_seconds __continuation_begin, const omniruntime::tzdb::__continuation &__continuation,
...
[[nodiscard]] static __sys_info_result __get_sys_info(date::sys_seconds __time, date::sys_seconds __continuation_begin,
    const omniruntime::tzdb::__continuation &__continuation, const __rules_storage_type &__rules_db)
{
    return std::visit([&](const auto &__value) {
        using _Tp = std::decay_t<decltype(__value)>;
        if constexpr (std::is_same_v<_Tp, std::string>)
            return __get_sys_info_rule(__time, __continuation_begin, __continuation, __get_rules(__rules_db, __value));
        else if constexpr (std::is_same_v<_Tp, std::monostate>)
            return __get_sys_info_basic(__time, __continuation_begin, __continuation, std::chrono::seconds(0));
        else if constexpr (std::is_same_v<_Tp, omniruntime::tzdb::__save>)
            return __get_sys_info_basic(__time, __continuation_begin, __continuation, __value.__time);
        else static_assert(False<_Tp>::value);
...
{
    if (__current.end != __next.begin) return false;

    if (__current.offset != __next.offset || __current.abbrev != __next.abbrev || __current.save != __next.save) return
        false;
...
[[nodiscard]] time_zone time_zone::__create(std::unique_ptr<time_zone::__impl> &&__p)
{
    if (__p == nullptr) {
        throw std::runtime_error("initialized time_zone without a valid pimpl object");
    }
    time_zone result;
    ...
    if (continuations.size() > 1 && (std::holds_alternative<
        std::string>(continuations[continuations.size() - 2].__rules) || std::holds_alternative<std::string>(
    ...
    if (continuations.back().__has_forever_rules) {
        ...
        const auto endTime = __from_to_sys_seconds(continuations.back().__stdoff, *std::get<1>(foreverRules),
...
[[nodiscard]] sys_info time_zone::__get_info_to_populate_transition(date::sys_seconds __time) const
{
    ...
    for (auto __it = __continuations.begin(); __it != __continuations.end(); ++__it) {
        ...
        if (date::year_month_day{std::chrono::floor<date::days>(__named_rule_until(__continuation).__until())}.year() +
            date::years(1) < date::year_month_day{std::chrono::floor<date::days>(__time)}.year()) {
            continue;
        }

        __sys_info_result __sys_info = __get_sys_info(__time, __continuation_begin, __continuation, __rules_db);
        ...
        if (__sys_info) {
            ...
            if (std::holds_alternative<std::string>(__continuation.__rules) && __sys_info->__can_merge && __sys_info->
                __info.begin + 12h > __sys_info->__info.end) {
            ...
            if (!__result) {
            ...
            } else if (__can_merge && __merge_continuation(*__result, __sys_info->__info)) {
                ...
                if (__valid_result) {
                    ...
                    if (__it == __continuations.begin()) {
                        throw std::runtime_error("the first rule should always seed the result");
                    }
                    ...
                    if (std::holds_alternative<std::string>(__last.__rules)) {
                        ...
                        if (__result->end != __sys_info->__info.begin) {
                            ...
                            date::sys_seconds __begin = __sys_info->__info.begin - __sys_info->__info.offset;
                            if (__end == __begin) {
    ...
    if (__valid_result) return *__result;
...
[[nodiscard]] sys_info time_zone::__get_info(date::sys_seconds __time) const
{
    ...
    if (iter == transitions.end() && __impl_->__continuations().back().__has_forever_rules) {
        ...
        date::year transitionYear = date::year_month_day{std::chrono::floor<date::days>(__time)}.year() -
            date::years(1);
        date::sys_seconds firstTransition = __rule_to_sys_seconds(continuation.__stdoff,
            std::get<1>(foreverRules)->__save.__time, *std::get<0>(foreverRules), transitionYear);
        date::sys_seconds secondTransition = __rule_to_sys_seconds(continuation.__stdoff,
            std::get<0>(foreverRules)->__save.__time, *std::get<1>(foreverRules), transitionYear);
        ...
        while (__time >= nextTransition) {
            ...
            for (int i = 0; i < 2 && __time >= nextTransition; i++) {
                ...
                nextTransition = __rule_to_sys_seconds(continuation.__stdoff, (*nextRule)->__save.__time,
                    *(*currentRule), transitionYear);
        ...
        return sys_info{
            ...
            std::chrono::duration_cast<std::chrono::minutes>((*currentRule)->__save.__time),
            __format(continuation, (*currentRule)->__letters, (*currentRule)->__save.__time)
        };
    }
...
[[nodiscard]] static bool __is_ambiguous(date::local_seconds __local_time, const sys_info &__first,
    const sys_info &__second)
{
    date::local_seconds __end_first{__first.end.time_since_epoch() + __first.offset};
    date::local_seconds __begin_second{__second.begin.time_since_epoch() + __second.offset};
...
[[nodiscard]] static local_info __get_info(date::local_seconds __local_time, const sys_info &__first,
    const sys_info &__second)
{
    date::local_seconds __end_first{__first.end.time_since_epoch() + __first.offset};
    date::local_seconds __begin_second{__second.begin.time_since_epoch() + __second.offset};
...
[[nodiscard]] local_info time_zone::__get_info(date::local_seconds __local_time) const
{
    ...
    if (__local_seconds < 0s && __info.offset > 0s)
        if (__local_seconds - date::local_seconds::min().time_since_epoch() < __info.offset) return {-1, __info, {}};

    if (__local_seconds > 0s && __info.offset < 0s)
        if (date::local_seconds::max().time_since_epoch() - __local_seconds < -__info.offset) return {-2, __info, {}};
    ...
    if (__sys_time < __info.begin)
        ...
        return omniruntime::tzdb::__get_info(__local_time, __get_info(__info.begin - 1s), __info);

    if (__sys_time >= __info.end)
        // Case 3 after __info
        return omniruntime::tzdb::__get_info(__local_time, __info, __get_info(__info.end));
    ...
    if (__info.begin != date::sys_days(date::year::min() / date::January / 1)) {
        ...
        if (__is_ambiguous(__local_time, __prev, __info)) return {local_info::ambiguous, __prev, __info};
    }
