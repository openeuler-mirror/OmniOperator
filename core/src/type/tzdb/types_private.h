...
struct __constrained_weekday {
    [[nodiscard]] date::year_month_day operator()(date::year __year, date::month __month) const
    {
        auto __result = static_cast<date::sys_days>(date::year_month_day{__year, __month, __day});
        date::weekday __wd{static_cast<date::sys_days>(__result)};

        if (__comparison == __le) __result -= __wd - __weekday;
        else __result += __weekday - __wd;
...
struct __at {
    ...
    omniruntime::tzdb::__clock __clock{omniruntime::tzdb::__clock::__local};
};
...
struct __rule {
    ...
    omniruntime::tzdb::__at __at;
    omniruntime::tzdb::__save __save;
    std::string __letters;
};

using __rules_storage_type = std::vector<std::pair<std::string, std::vector<__rule>>>; // TODO TZDB use
// flat_map;
...
struct __continuation {
    ...
    using __rules_t = std::variant<std::monostate, omniruntime::tzdb::__save, std::string /*, size_t*/>;
    ...
    omniruntime::tzdb::__on __on{date::day{1}};
    omniruntime::tzdb::__at __at{std::chrono::seconds{0}, omniruntime::tzdb::__clock::__local};
};
} // namespace omniruntime::tzdb
