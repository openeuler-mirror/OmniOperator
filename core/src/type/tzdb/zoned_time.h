...
struct zoned_traits<const time_zone *> {
    ...
    {
        return omniruntime::tzdb::locate_zone(__name);
    }
};
...
public:
    ...
    zoned_time(_TimeZonePtr __zone, const date::local_time<_Duration> &__tp, choose __c)
        : __zone_{std::move(__zone)}, __tp_{__zone_->to_sys(__tp, __c)} {}

    zoned_time(std::string_view __name, const date::local_time<_Duration> &__tp, choose __c)
    ...
    template <class _Duration2, class _TimeZonePtr2>
    zoned_time(std::string_view __name, const zoned_time<_Duration2, _TimeZonePtr2> &__zt)
        : zoned_time{__traits::locate_zone(__name), __zt} {}

    template <class _Duration2, class _TimeZonePtr2>
    zoned_time(std::string_view __name, const zoned_time<_Duration2, _TimeZonePtr2> &__zt, choose __c)
...
zoned_time(date::sys_time<_Duration>) -> zoned_time<std::common_type_t<_Duration, std::chrono::seconds>>;

template <class _TimeZonePtrOrName>
using __time_zone_representation = std::conditional_t<std::is_convertible_v<_TimeZonePtrOrName, std::string_view>, const
    time_zone *, folly::remove_cvref_t<_TimeZonePtrOrName>>;

template <class _TimeZonePtrOrName>
zoned_time(_TimeZonePtrOrName &&) -> zoned_time<std::chrono::seconds, __time_zone_representation<_TimeZonePtrOrName>>;

template <class _TimeZonePtrOrName, class _Duration>
zoned_time(_TimeZonePtrOrName &&,
    date::sys_time<_Duration>) -> zoned_time<std::common_type_t<_Duration, std::chrono::seconds>,
    __time_zone_representation<_TimeZonePtrOrName>>;

template <class _TimeZonePtrOrName, class _Duration>
zoned_time(_TimeZonePtrOrName &&, date::local_time<_Duration>,
    choose = choose::earliest) -> zoned_time<std::common_type_t<_Duration, std::chrono::seconds>,
    __time_zone_representation<_TimeZonePtrOrName>>;
