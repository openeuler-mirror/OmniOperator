...
public:
    ...
    [[nodiscard]] sys_info get_info(const date::sys_time<_Duration> &__time) const
    {
        return __get_info(std::chrono::time_point_cast<std::chrono::seconds>(__time));
    }

    template <class _Duration>
    [[nodiscard]] local_info get_info(const date::local_time<_Duration> &__time) const
    {
        return __get_info(std::chrono::time_point_cast<std::chrono::seconds>(__time));
    }
        ...
        const date::local_time<_Duration> &__time) const
    {
        ...
        switch (__info.result) {
            case local_info::unique:
                return date::sys_time<std::common_type_t<_Duration, std::chrono::seconds>>{
        ...
        if (__info.result == -2) {
            throw std::runtime_error("cannot convert the local time; it would be after the maximum system clock value");
        }
    ...
    template <class _Duration>
    [[nodiscard]] date::sys_time<std::common_type_t<_Duration, std::chrono::seconds>>
    ...
    template <class _Duration>
    [[nodiscard]] date::local_time<std::common_type_t<_Duration, std::chrono::seconds>>
    to_local(const date::sys_time<_Duration> &__time) const
    {
        ...
        if (__info.offset < std::chrono::seconds{0} && __time.time_since_epoch() < _Dp::min() - __info.offset) {
            throw std::runtime_error(
                "cannot convert the system time; it would be before the minimum local clock value");
        }

        if (__info.offset > std::chrono::seconds{0} && __time.time_since_epoch() > _Dp::max() - __info.offset) {
            throw std::runtime_error("cannot convert the system time; it would be after the maximum local clock value");
        }
...
[[nodiscard]] inline bool operator==(const time_zone &__x, const time_zone &__y) noexcept
{
