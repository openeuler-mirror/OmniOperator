...
public:
    ...
    virtual std::unique_ptr<Aggregator> CreateAggregator(const DataTypes &inputTypes, const DataTypes &outputTypes,
        std::vector<int32_t> &channels, bool inputRaw, bool outputPartial, bool isOverflowAsNull) = 0;
};
...
protected:
        ...
        std::vector<int32_t> &channels)
    {
        ...
        switch (inputTypeId) {
            case OMNI_BOOLEAN:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_BOOLEAN, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_SHORT:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_SHORT, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_DATE32:
            case OMNI_TIME32:
            case OMNI_INT:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_INT, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_LONG:
            case OMNI_DATE64:
            case OMNI_TIME64:
            case OMNI_TIMESTAMP:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_LONG, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_DOUBLE:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_DOUBLE, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_DECIMAL64:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_DECIMAL64, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_DECIMAL128:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_DECIMAL128, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_CONTAINER:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_CONTAINER, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_VARCHAR:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_VARCHAR, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_CHAR:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_CHAR, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_NONE:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_NONE, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            case OMNI_INVALID:
                return T<RAW_IN, PARTIAL_OUT, NULL_OVERFLOW, OMNI_INVALID, OUT_ID>::Create(std::move(inputTypes),
                    std::move(outputTypes), channels);
            default:
...
public:
        ...
        bool isOverflowAsNull = true) override
    {
        ...
        switch (inputTypeId) {
            ...
            case OMNI_INT: {
                return CreateAggregatorHelper<SumFlatIMAggregator, IntVector, LongVector, int64_t>(inputTypes,
                    outputTypes, channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_LONG: {
                return CreateAggregatorHelper<SumFlatIMAggregator, LongVector, LongVector, int64_t>(inputTypes,
                    outputTypes, channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_DOUBLE: {
...
public:
        ...
        bool isOverflowAsNull = true) override
    {
        ...
        switch (inputTypeId) {
            ...
            case OMNI_DECIMAL64:
            case OMNI_DECIMAL128: {
                return CreateAggregatorHelper<AverageSparkDecimalAggregator>(inputTypes, outputTypes, channels,
                    inputRaw, outputPartial, isOverflowAsNull);
            }
            default: {
...
public:
        ...
        bool isOverflowAsNull = true)
    {
        if (inputRaw) {
            if (outputPartial) {
                ...
                return std::make_unique<FirstAggregator<true, false, InputVecType, InputType>>(aggregateType,
                    inputTypes, outputTypes, channels, inputRaw, outputPartial, isOverflowAsNull);
            }
        } else {
            if (outputPartial) {
                return std::make_unique<FirstAggregator<false, true, InputVecType, InputType>>(aggregateType,
                    inputTypes, outputTypes, channels, inputRaw, outputPartial, isOverflowAsNull);
            } else {
                return std::make_unique<FirstAggregator<false, false, InputVecType, InputType>>(aggregateType,
                    inputTypes, outputTypes, channels, inputRaw, outputPartial, isOverflowAsNull);
            }
        }
    }
        ...
        bool isOverflowAsNull = false) override
    {
        ...
        switch (inputTypeId) {
            case OMNI_BOOLEAN: {
                return CreateFirstAggregatorHelper<BooleanVector, bool>(aggregateType, inputTypes, outputTypes,
                    channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_SHORT: {
                return CreateFirstAggregatorHelper<ShortVector, int16_t>(aggregateType, inputTypes, outputTypes,
                    channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_INT:
            case OMNI_DATE32: {
            ...
            case OMNI_LONG:
            case OMNI_DECIMAL64: {
                return CreateFirstAggregatorHelper<LongVector, int64_t>(aggregateType, inputTypes, outputTypes,
                    channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_DOUBLE: {
                return CreateFirstAggregatorHelper<DoubleVector, double>(aggregateType, inputTypes, outputTypes,
                    channels, inputRaw, outputPartial, isOverflowAsNull);
            }
            case OMNI_DECIMAL128: {
