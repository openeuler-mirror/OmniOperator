...
RegrCountAggregator::RegrCountAggregator(const DataTypes &inputTypes, const DataTypes &outputTypes,
    std::vector<int32_t> &channels, bool inputRaw, bool outputPartial, bool isOverflowAsNull)
    : TypedAggregator(OMNI_AGGREGATION_TYPE_REGR_COUNT, inputTypes, outputTypes, channels, inputRaw, outputPartial,
          isOverflowAsNull)
{
}

size_t RegrCountAggregator::GetStateSize()
{
    return sizeof(RegrState);
}

void RegrCountAggregator::ProcessSingleInternal(AggregateState *state, BaseVector *vector, const int32_t rowOffset,
...
void RegrCountAggregator::ProcessGroupInternal(std::vector<AggregateState *> &rowStates, BaseVector *vector,
...
void RegrCountAggregator::InitState(AggregateState *state)
{
...
void RegrCountAggregator::InitStates(std::vector<AggregateState *> &groupStates)
{
    for (auto *s : groupStates)
        InitState(s);
}

void RegrCountAggregator::ExtractValues(const AggregateState *state, std::vector<BaseVector *> &vectors,
    const int32_t rowIndex)
{
    ...
    double result = static_cast<double>(s->count);
...
void RegrCountAggregator::ExtractValuesBatch(std::vector<AggregateState *> &groupStates,
    std::vector<BaseVector *> &vectors, int32_t rowOffset, int32_t rowCount)
{
    ...
    for (int32_t i = 0; i < rowCount; i++) {
        ...
        double result = static_cast<double>(s->count);
        if (outputTypes.GetType(0)->GetId() == OMNI_LONG) {
            static_cast<Vector<int64_t> *>(outVec)->SetValue(row, static_cast<int64_t>(result));
        } else {
...
void RegrCountAggregator::ExtractValuesForSpill(std::vector<AggregateState *> &groupStates,
...
void RegrCountAggregator::ProcessGroupUnspill(std::vector<UnspillRowInfo> &unspillRows, int32_t rowCount,
    int32_t &vectorIndex)
{
...
void RegrCountAggregator::AlignAggSchema(VectorBatch *result, VectorBatch *inputVecBatch)
{
...
void RegrCountAggregator::AlignAggSchemaWithFilter(VectorBatch *result, VectorBatch *inputVecBatch,
...
void RegrCountAggregator::ProcessAlignAggSchema(VectorBatch *result, BaseVector *originVector,
...
std::vector<DataTypePtr> RegrCountAggregator::GetSpillType()
{
