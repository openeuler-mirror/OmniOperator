 ...
 */
#include "operator/aggregation/aggregator/regr/regr_avgx_aggregator.h"
...
static void ExtractPartialBatch(const omniruntime::type::DataTypes &,
...
RegrAvgXAggregator::RegrAvgXAggregator(const DataTypes &inputTypes, const DataTypes &outputTypes,
    std::vector<int32_t> &channels, bool inputRaw, bool outputPartial, bool isOverflowAsNull)
    : TypedAggregator(OMNI_AGGREGATION_TYPE_REGR_AVGX, inputTypes, outputTypes, channels, inputRaw, outputPartial,
          isOverflowAsNull)
{
}

size_t RegrAvgXAggregator::GetStateSize()
{
    return sizeof(RegrState);
}

void RegrAvgXAggregator::ProcessSingleInternal(AggregateState *state, BaseVector *vector, const int32_t rowOffset,
...
void RegrAvgXAggregator::ProcessGroupInternal(std::vector<AggregateState *> &rowStates, BaseVector *vector,
...
void RegrAvgXAggregator::InitState(AggregateState *state)
{
...
void RegrAvgXAggregator::InitStates(std::vector<AggregateState *> &groupStates)
{
    for (auto *s : groupStates)
        InitState(s);
}

void RegrAvgXAggregator::ExtractValues(const AggregateState *state, std::vector<BaseVector *> &vectors,
    const int32_t rowIndex)
{
    ...
    double result = s->meanX;
    if (result == 0.0)
        result = 0.0;
    static_cast<Vector<double> *>(outVec)->SetValue(rowIndex, result);
}

void RegrAvgXAggregator::ExtractValuesBatch(std::vector<AggregateState *> &groupStates,
...
void RegrAvgXAggregator::ExtractValuesForSpill(std::vector<AggregateState *> &groupStates,
...
void RegrAvgXAggregator::ProcessGroupUnspill(std::vector<UnspillRowInfo> &unspillRows, int32_t rowCount,
    int32_t &vectorIndex)
{
...
void RegrAvgXAggregator::AlignAggSchema(VectorBatch *result, VectorBatch *inputVecBatch)
{
...
void RegrAvgXAggregator::AlignAggSchemaWithFilter(VectorBatch *result, VectorBatch *inputVecBatch,
...
void RegrAvgXAggregator::ProcessAlignAggSchema(VectorBatch *result, BaseVector *originVector,
...
std::vector<DataTypePtr> RegrAvgXAggregator::GetSpillType()
{
