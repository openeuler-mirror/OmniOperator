 ...
 * Description: regr_sxy(y, x) - full aggregator (raw / merge / partial / final / spill) in this pair of files.
 */
#ifndef OMNI_RUNTIME_REGR_SXY_AGGREGATOR_H
#define OMNI_RUNTIME_REGR_SXY_AGGREGATOR_H
...
class RegrSxyAggregator : public TypedAggregator {
public:
    RegrSxyAggregator(const DataTypes &inputTypes, const DataTypes &outputTypes, std::vector<int32_t> &channels,
        bool inputRaw, bool outputPartial, bool isOverflowAsNull);
    ~RegrSxyAggregator() override = default;
...
#endif // OMNI_RUNTIME_REGR_SXY_AGGREGATOR_H
