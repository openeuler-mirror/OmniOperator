 ...
 * Description: regr_sxx(y, x) - full aggregator (raw / merge / partial / final / spill) in this pair of files.
 */
#ifndef OMNI_RUNTIME_REGR_SXX_AGGREGATOR_H
#define OMNI_RUNTIME_REGR_SXX_AGGREGATOR_H
...
class RegrSxxAggregator : public TypedAggregator {
public:
    RegrSxxAggregator(const DataTypes &inputTypes, const DataTypes &outputTypes, std::vector<int32_t> &channels,
        bool inputRaw, bool outputPartial, bool isOverflowAsNull);
    ~RegrSxxAggregator() override = default;
...
#endif // OMNI_RUNTIME_REGR_SXX_AGGREGATOR_H
