...
void CovarSampAggregator<IN_ID, OUT_ID>::ExtractValues(const AggregateState *state, std::vector<BaseVector *> &vectors,
    ...
    if (s->IsOverFlowed()) {
        if (outputPartial) {
            ...
            this->SetNullOrThrowException(vectors[0], rowIndex, "covar_samp_aggregator overflow.");
        } else {
            this->SetNullOrThrowException(vectors[0], rowIndex, "covar_samp_aggregator overflow.");
        }
        return;
    ...
    if (outputPartial) {
        ...
        // Spark: covar_samp has no result when count < 2; return NULL.
        if (s->count < 2) {
            static_cast<OutVector *>(vectors[0])->SetNull(rowIndex);
            return;
        }
    ...
    if (outputPartial) {
        ...
        for (int32_t i = 0; i < rowCount; i++) {
            ...
            if (s->IsEmpty()) {
                ...
                vN->SetNull(i); vXAvg->SetNull(i); vYAvg->SetNull(i); vCk->SetNull(i);
                this->SetNullOrThrowException(vectors.size() >= kCovarPartialColumnCount ? vectors[0] : reinterpret_cast<BaseVector *>(vN), i, "covar_samp_aggregator overflow.");
            } else {
        ...
        for (int32_t i = 0; i < rowCount; i++) {
            ...
            if (s->IsEmpty()) {
                ...
                v->SetNull(i); // Spark: covar_samp has no result when count < 2
            } else if (s->IsOverFlowed()) {
                this->SetNullOrThrowException(v, i, "covar_samp_aggregator overflow.");
            } else {
                double val = CovarSampFromState(s->count, s->meanX, s->meanY, s->c2);
                if (!std::isfinite(val)) {
                    this->SetNullOrThrowException(v, i, "covar_samp_aggregator overflow.");
                } else {
...
template <DataTypeId IN_ID, DataTypeId OUT_ID>
void CovarSampAggregator<IN_ID, OUT_ID>::ExtractValuesForSpill(std::vector<AggregateState *> &groupStates,
...
template <DataTypeId IN_ID, DataTypeId OUT_ID>
void CovarSampAggregator<IN_ID, OUT_ID>::ProcessGroupUnspill(std::vector<UnspillRowInfo> &unspillRows, int32_t rowCount,
    int32_t &vectorIndex) {
...
template <DataTypeId IN_ID, DataTypeId OUT_ID>
void CovarSampAggregator<IN_ID, OUT_ID>::AlignAggSchema(VectorBatch *result, VectorBatch *inputVecBatch)
{
...
template <DataTypeId IN_ID, DataTypeId OUT_ID>
void CovarSampAggregator<IN_ID, OUT_ID>::AlignAggSchemaWithFilter(VectorBatch *result, VectorBatch *inputVecBatch,
...
template <DataTypeId IN_ID, DataTypeId OUT_ID>
void CovarSampAggregator<IN_ID, OUT_ID>::ProcessAlignAggSchema(VectorBatch *result, BaseVector *originVector,
