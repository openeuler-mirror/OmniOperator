...
public:
    MinVarcharAggregator(const DataType &in, const DataType &out, int32_t channel)
        : Aggregator(OMNI_AGGREGATION_TYPE_MIN, in, out, channel)
    {}

    MinVarcharAggregator(const DataType &in, const DataType &out, int32_t channel, bool inputRaw, bool outputPartial)
