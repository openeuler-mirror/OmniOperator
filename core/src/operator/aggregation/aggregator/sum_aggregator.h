...
namespace op {
    template<typename V, typename IN, typename ResultType>
    ...
    public:
        ...
        void ProcessNonGroup(Vector *colPtr, uint32_t offset) override
        {
            if (UNLIKELY(colPtr->IsValueNull(offset))) {
                return;
            }
            ...
            if (nonGroupState.val == nullptr) {
                InitiateNonGroup(colPtr, offset);
                return;
            }
            *(static_cast<ResultType *>(nonGroupState.val)) += (static_cast<V *>(colPtr))->GetValue(offset);
        }
        ...
        void InitiateNonGroup(Vector *colPtr, uint32_t offset) override
        {
            if (UNLIKELY(colPtr->IsValueNull(offset))) {
                return;
            }
