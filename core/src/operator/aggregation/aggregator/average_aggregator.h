...
public:
    AverageAggregator(const DataTypePtr in, const DataTypePtr out, int32_t channel)
        : Aggregator(OMNI_AGGREGATION_TYPE_AVG, in, out, channel)
    {}

    AverageAggregator(const DataTypePtr in, const DataTypePtr out, int32_t channel, bool inputRaw, bool outputPartial)
