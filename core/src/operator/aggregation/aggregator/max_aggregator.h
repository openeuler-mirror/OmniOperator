...
public:
    MaxAggregator(const DataTypePtr in, const DataTypePtr out, int32_t channel)
        : Aggregator(OMNI_AGGREGATION_TYPE_MAX, in, out, channel)
    {}

    MaxAggregator(const DataTypePtr in, const DataTypePtr out, int32_t channel, bool inputRaw, bool outputPartial)
