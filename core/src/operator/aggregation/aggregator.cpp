    ...
    uint32_t offset)
{
    ...
    SumAggregator::SUM_FUNCTIONS[typeId].insertFunc(groupSlot, colPtr, type, rowIdx);
}
...
void SumAggregator::ProcessNonGroup(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    auto typeId = static_cast<VecTypeId>(type);
    SumAggregator::SUM_FUNCTIONS[typeId].processNonGroupFunc(nonGroupState, colPtr, type, rowIdx);
}
...
void CountAggregator::ProcessNonGroup(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    colPtr = VectorHelper::GetDictionary(colPtr, rowIdx);
    if (!initiated) {
        Initiate(colPtr, type, rowIdx);
        return;
    }
    if (inputRaw) {
        nonGroupState.count++;
    } else {
        nonGroupState.count += reinterpret_cast<LongVector *>(colPtr)->GetValue(rowIdx);
    }
}
...
void AverageAggregator::Initiate(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    int32_t rowIdx = offset;
    colPtr = VectorHelper::GetDictionary(colPtr, rowIdx);
    AverageAggregator::AVG_FUNCTIONS[typeId].initiateFunc(nonGroupState, colPtr, type, rowIdx);
    initiated = true;
}
...
void AverageAggregator::ProcessNonGroup(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    auto typeId = static_cast<VecTypeId>(type);
    AverageAggregator::AVG_FUNCTIONS[typeId].processNonGroupFunc(nonGroupState, colPtr, type, rowIdx);
}
...
void AverageAggregator::Insert(GroupBySlot &groupSlot, Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    int32_t rowIdx = offset;
    colPtr = VectorHelper::GetDictionary(colPtr, rowIdx);
    AverageAggregator::AVG_FUNCTIONS[typeId].insertFunc(groupSlot, colPtr, type, rowIdx);
}
...
void MinAggregator::Insert(GroupBySlot &groupSlot, Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    colPtr = VectorHelper::GetDictionary(colPtr, rowIdx);
    auto typeId = static_cast<VecTypeId>(type);
    MinAggregator::MIN_FUNCTIONS[typeId].insertFunc(groupSlot, colPtr, type, rowIdx);
}
...
void MinAggregator::ProcessNonGroup(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    auto typeId = static_cast<VecTypeId>(type);
    MinAggregator::MIN_FUNCTIONS[typeId].processNonGroupFunc(nonGroupState, colPtr, type, rowIdx);
}
...
void MaxAggregator::Insert(GroupBySlot &groupSlot, Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    colPtr = VectorHelper::GetDictionary(colPtr, rowIdx);
    auto typeId = static_cast<VecTypeId>(type);
    MaxAggregator::MAX_FUNCTIONS[typeId].insertFunc(groupSlot, colPtr, type, rowIdx);
}
...
void MaxAggregator::ProcessNonGroup(Vector *colPtr, int32_t type, uint32_t offset)
{
    ...
    auto typeId = static_cast<VecTypeId>(type);
    MaxAggregator::MAX_FUNCTIONS[typeId].processNonGroupFunc(nonGroupState, colPtr, type, rowIdx);
}
