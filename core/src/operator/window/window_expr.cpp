...
WindowWithExprOperatorFactory::WindowWithExprOperatorFactory(const vec::VecTypes &sourceTypes,
    ...
    const std::vector<omniruntime::expressions::Expr *> &argumentKeys, int32_t argumentChannelsCount)
{
    ...
    this->operatorFactory = WindowOperatorFactory::CreateWindowOperatorFactory(*(this->sourceTypes.get()),
        ...
        preGroupedCols, preGroupedCount, sortCols, sortAscendings, sortNullFirsts, sortColCount,
        preSortedChannelPrefix, expectedPositions, allTypes, this->argumentChannels.data(),
    ...
    const std::vector<omniruntime::expressions::Expr *> &argumentKeys, int32_t argumentChannelsCount)
{
    auto factory = std::make_unique<WindowWithExprOperatorFactory>(sourceTypes, outputCols, outputColsCount,
        ...
        outputVecTypes, argumentKeys, argumentChannelsCount);
...
Operator *WindowWithExprOperatorFactory::CreateOperator()
{
    ...
    auto windowWithExprOperator = std::make_unique<WindowWithExprOperator>(*(sourceTypes.get()), argumentChannels,
        projectFuncs, windowOperator);
    return windowWithExprOperator.release();
}

WindowWithExprOperator::WindowWithExprOperator(const vec::VecTypes &sourceTypes,
