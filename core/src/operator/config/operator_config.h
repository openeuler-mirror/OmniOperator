...
namespace op {
enum SpillConfigId { SPILL_CONFIG_NONE = 0, SPILL_CONFIG_OLK = 1, SPILL_CONFIG_SPARK = 2, SPILL_CONFIG_INVALID };
...
public:
    ...
    virtual bool NeedSpill(MemoryBuilder *memoryBuilder) { return false; }

    virtual bool NeedSpill(size_t elementsSize) { return false; }

    SpillConfigId GetSpillConfigId() { return spillConfigId; }

    bool IsSpillEnabled() const { return spillEnabled; }

    std::string &GetSpillPath() { return spillPath; }

    uint64_t GetMaxSpillBytes() const { return maxSpillBytes; }

    void SetMaxSpillBytes(uint64_t maxSpillSize) { this->maxSpillBytes = maxSpillSize; }

    uint64_t GetWriteBufferSize() const { return writeBufferSize; }
...
public:
    SparkSpillConfig(bool enabled, const std::string &spillPath, uint64_t maxSpillBytes, int32_t numElementsThreshold,
                     double memUsageFraction, uint64_t writeBufferSize)
    ...
    {
        OMNI_CHECK(memUsageFraction > 0.0 && memUsageFraction <= 1.0, "Spill memory fraction must be in (0, 1]");
        ...
        if (limit == mem::MemoryManager::UNLIMIT) {
            ...
            memUsageForSpillThreshold =
                memUsageFraction == 1.0 ? limit : static_cast<int64_t>(limit * memUsageFraction);
        }
    }

    SparkSpillConfig(const std::string &spillPath, uint64_t maxSpillBytes, int32_t numElementsThreshold,
                     double memUsageFraction = 0.9, uint64_t writeBufferSize = 0)
        : SparkSpillConfig(true, spillPath, maxSpillBytes, numElementsThreshold, memUsageFraction, writeBufferSize)
    {
    }
    ...
    int32_t GetSpillRowThreshold() const { return numElementsForSpillThreshold; }

    int64_t GetSpillMemThreshold() const { return memUsageForSpillThreshold; }
...
enum OverflowConfigId { OVERFLOW_CONFIG_EXCEPTION = 0, OVERFLOW_CONFIG_NULL = 1 };
...
public:
    ...
    OverflowConfigId GetOverflowConfigId() const { return overflowConfigId; }

    bool IsOverflowAsNull() const { return overflowConfigId == OVERFLOW_CONFIG_NULL; }
...
public:
    OperatorConfig()
        : spillConfig(new SpillConfig()), overflowConfig(new OverflowConfig()), isSkipVerify(false),
    ...
    OperatorConfig(SpillConfig *spillConfig, OverflowConfig *overflowConfig, bool isSkipVerify,
        ...
        : spillConfig((spillConfig != nullptr) ? spillConfig : new SpillConfig()),
          ...
          isSkipVerify(isSkipVerify), adaptivityThreshold(adaptivityThreshold), isRowOutput(isRowOutput),
    ...
    OperatorConfig(SpillConfig *spillConfig, OverflowConfig *overflowConfig)
        : spillConfig((spillConfig != nullptr) ? spillConfig : new SpillConfig()),
          overflowConfig((overflowConfig != nullptr) ? overflowConfig : new OverflowConfig()), isSkipVerify(false)
    {
    }
    ...
    SpillConfig *GetSpillConfig() const { return spillConfig; }

    void SetSpillConfig(SpillConfig *pSpillConfig) { this->spillConfig = pSpillConfig; }

    OverflowConfig *GetOverflowConfig() const { return overflowConfig; }

    void SetOverflowConfig(OverflowConfig *pOverflowConfig) { this->overflowConfig = pOverflowConfig; }

    bool IsSkipVerify() const { return isSkipVerify; }

    void SetSkipVerify(bool needSkipVerify) { this->isSkipVerify = needSkipVerify; }

    int GetAdaptivityThreshold() const { return adaptivityThreshold; }

    bool IsRowOutput() const { return isRowOutput; }

    bool IsStatisticalAggregate() const { return isStatisticalAggregate; }
