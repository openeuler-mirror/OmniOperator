...
OperatorConfig::OperatorConfig(const OperatorConfig &operatorConfig)
{
    ...
    this->isRowOutput = operatorConfig.GetIsRowOutput();
}
...
OperatorConfig OperatorConfig::DeserializeOperatorConfig(const std::string &configString)
{
    ...
    if (result.contains("rowOutput")) {
        isRowOutput = result.at("rowOutput").get<bool>();
    }

    return OperatorConfig { resultSpillConfig, resultOverflowConfig, needSkipVerify, adaptThreshold, isRowOutput};
}
