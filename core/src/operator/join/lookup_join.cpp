        ...
        if (result.IsInsert()) {
            ...
            while (it.IsOk()) {
                ...
                if constexpr (hasJoinFilter) {
                    auto filterResult = IsJoinPositionEligible(partitionMask, address, probePosition, contextPtr);
                    if (filterResult) {
                        if constexpr (joinType != OMNI_JOIN_TYPE_LEFT_ANTI) {
                            outputBuilder->AppendRow(probePosition, buildColumns, address);
                        }
                        if constexpr (joinType == OMNI_JOIN_TYPE_FULL || joinType == OMNI_JOIN_TYPE_RIGHT) {
                                                  ...
                                                  ExecutionContext *contextPtr, BaseVector ***buildColumns)
{
    ...
    for (int32_t j = 0; j < matchRowCnt; j++) {
        ...
        while (it.IsOk()) {
            auto address = LookupJoinOutputBuilder::EncodeAddress(it->rowIdx, it->vecBatchIdx);
            if constexpr (hasJoinFilter) {
                auto filterResult = IsJoinPositionEligible(partitionMask, address, rowIndex, contextPtr);
                if (filterResult) {
                    if constexpr (joinType != OMNI_JOIN_TYPE_LEFT_ANTI) {
...
void LookupJoinOutputBuilder::BuildOutput(BaseVector **probeOutputColumns, VectorBatch **outputVecBatch)
{
    ...
    const std::tuple<int32_t, BaseVector ***, uint32_t, uint32_t> *tempIndex = probeBuildIndex.data() + probeRowOffset;
