 ...
 * @Description: Use the simd instruction to implement the sorting of less than 16 numbers.
 */
    ...
    if constexpr (DataType == SIMD_DOUBLE) {
        if constexpr (SortingRule == SORTASCENDING) {
            if constexpr (std::is_same_v<VType, int64x2_t>) {
                ...
                uint64x1_t lessThan = vclt_f64(vreinterpret_f64_s64(a), vreinterpret_f64_s64(b));
...
template <class VType, int32_t SortRule, int32_t DataType> static void ALWAYS_INLINE Sort2(VType &a, VType &b)
{
    ...
    b = Max64<VType, SortRule, DataType>(copyA, b);
}
...
static void ALWAYS_INLINE Sort2WithAddr(VType &a, VType &b, AddrType &c, AddrType &d)
{
    ...
    if constexpr (std::is_same_v<VType, int64x1_t>) {
        ...
        if (vgetq_lane_s64(a, 1) != vgetq_lane_s64(copyA, 1)) {
...
{
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v0, v2, vAddr0, vAddr2);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v1, v3, vAddr1, vAddr3);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v4, v6, vAddr4, vAddr6);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v5, v7, vAddr5, vAddr7);

    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v0, v4, vAddr0, vAddr4);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v1, v5, vAddr1, vAddr5);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v2, v6, vAddr2, vAddr6);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v3, v7, vAddr3, vAddr7);

    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v0, v1, vAddr0, vAddr1);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v2, v3, vAddr2, vAddr3);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v4, v5, vAddr4, vAddr5);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v6, v7, vAddr6, vAddr7);

    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v2, v4, vAddr2, vAddr4);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v3, v5, vAddr3, vAddr5);

    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v1, v4, vAddr1, vAddr4);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v3, v6, vAddr3, vAddr6);

    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v1, v2, vAddr1, vAddr2);
    Sort2WithAddr<VType, AddrType, SortRule, DataType>(v3, v4, vAddr3, vAddr4);
...
static void ALWAYS_INLINE Sort8Rows(int64_t *array, uint64_t *arrayAddr, int32_t dataLen)
{
    ...
    Sort8<int64x2_t, uint64x2_t, SortRule, DataType>(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], vAddr[0], vAddr[1],
        vAddr[2], vAddr[3], vAddr[4], vAddr[5], vAddr[6], vAddr[7]);
    Merge8x2<SortRule, DataType>(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], vAddr[0], vAddr[1], vAddr[2], vAddr[3],
    ...
    uint64x2_t &vAddrc, uint64x2_t &vAddrd, uint64x2_t &vAddre, uint64x2_t &vAddrf)
{
    ...
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(va, vb, vAddra, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vc, vd, vAddrc, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(ve, vf, vAddre, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v0, v2, vAddr0, vAddr2);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, v3, vAddr1, vAddr3);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v4, v6, vAddr4, vAddr6);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, v7, vAddr5, vAddr7);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v8, va, vAddr8, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, vb, vAddr9, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vc, ve, vAddrc, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vd, vf, vAddrd, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v0, v4, vAddr0, vAddr4);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, v5, vAddr1, vAddr5);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, v6, vAddr2, vAddr6);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, v7, vAddr3, vAddr7);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v8, vc, vAddr8, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, vd, vAddr9, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(va, ve, vAddra, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vb, vf, vAddrb, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v0, v8, vAddr0, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, v9, vAddr1, vAddr9);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, va, vAddr2, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, vb, vAddr3, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v4, vc, vAddr4, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, vd, vAddr5, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v6, ve, vAddr6, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, vf, vAddr7, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, va, vAddr5, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v6, v9, vAddr6, vAddr9);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, vc, vAddr3, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, vb, vAddr7, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vd, ve, vAddrd, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v4, v8, vAddr4, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, v2, vAddr1, vAddr2);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, v4, vAddr1, vAddr4);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, vd, vAddr7, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, v8, vAddr2, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vb, ve, vAddrb, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, v4, vAddr2, vAddr4);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, v6, vAddr5, vAddr6);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, va, vAddr9, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vb, vd, vAddrb, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, v8, vAddr3, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, vc, vAddr7, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, v5, vAddr3, vAddr5);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v6, v8, vAddr6, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, v9, vAddr7, vAddr9);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(va, vc, vAddra, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, v4, vAddr3, vAddr4);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, v6, vAddr5, vAddr6);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, v8, vAddr7, vAddr8);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, va, vAddr9, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vb, vc, vAddrb, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v6, v7, vAddr6, vAddr7);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v8, v9, vAddr8, vAddr9);
}
    ...
    uint64x2_t &vAddrc, uint64x2_t &vAddrd, uint64x2_t &vAddre, uint64x2_t &vAddrf)
{
    ...
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v0, vf, vAddr0, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v1, ve, vAddr1, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, vd, vAddr2, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, vc, vAddr3, vAddrc);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v4, vb, vAddr4, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, va, vAddr5, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v6, v9, vAddr6, vAddr9);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v7, v8, vAddr7, vAddr8);
    v7 = ReverseKeys2(v7);
    ...
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v2, v5, vAddr2, vAddr5);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v3, v4, vAddr3, vAddr4);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v8, vf, vAddr8, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, ve, vAddr9, vAddre);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(va, vd, vAddra, vAddrd);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vb, vc, vAddrb, vAddrc);
    v3 = ReverseKeys2(v3);
    vAddr3 = ReverseAddrs2(vAddr3);
    v2 = ReverseKeys2(v2);
    ...
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v4, v7, vAddr4, vAddr7);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v5, v6, vAddr5, vAddr6);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v8, vb, vAddr8, vAddrb);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(v9, va, vAddr9, vAddra);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vc, vf, vAddrc, vAddrf);
    Sort2WithAddr<int64x2_t, uint64x2_t, SortRule, DataType>(vd, ve, vAddrd, vAddre);
    v1 = ReverseKeys2(v1);
    vAddr1 = ReverseAddrs2(vAddr1);
    v3 = ReverseKeys2(v3);
    ...
    v6 = SortPair64<SortRule, DataType>(v6, vAddr6);
    v7 = SortPair64<SortRule, DataType>(v7, vAddr7);
    v8 = SortPair64<SortRule, DataType>(v8, vAddr8);
    v9 = SortPair64<SortRule, DataType>(v9, vAddr9);
    va = SortPair64<SortRule, DataType>(va, vAddra);
    vb = SortPair64<SortRule, DataType>(vb, vAddrb);
    vc = SortPair64<SortRule, DataType>(vc, vAddrc);
    vd = SortPair64<SortRule, DataType>(vd, vAddrd);
    ve = SortPair64<SortRule, DataType>(ve, vAddre);
...
static void ALWAYS_INLINE Sort16Rows(int64_t *array, uint64_t *arrayAddr, int32_t dataLen)
{
    ...
    Sort16<SortRule, DataType>(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13],
        v[14], v[15], vAddr[0], vAddr[1], vAddr[2], vAddr[3], vAddr[4], vAddr[5], vAddr[6], vAddr[7], vAddr[8],
        vAddr[9], vAddr[10], vAddr[11], vAddr[12], vAddr[13], vAddr[14], vAddr[15]);
    Merge16x2<SortRule, DataType>(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12],
        v[13], v[14], v[15], vAddr[0], vAddr[1], vAddr[2], vAddr[3], vAddr[4], vAddr[5], vAddr[6], vAddr[7], vAddr[8],
        vAddr[9], vAddr[10], vAddr[11], vAddr[12], vAddr[13], vAddr[14], vAddr[15]);
    for (i = 0, index = 0; index + 2 <= dataLen; index += 2, i++) {
        vst1q_s64(array + index, v[i]);
        vst1q_u64(arrayAddr + index, vAddr[i]);
...
static void ALWAYS_INLINE Sort2Rows(int64_t *array, uint64_t *arrayAddr, int32_t dataLen)
{
    ...
    Sort2WithAddr<int64x1_t, uint64x1_t, SortRule, DataType>(v0, v1, vAddr0, vAddr1);
    vst1_s64(array, v0);
    vst1_s64(array + 1, v1);
...
template <int32_t SortRule, int32_t DataType> static void ALWAYS_INLINE Sort3Rows(int64_t *array, int32_t dataLen)
{
    ...
    Sort3<SortRule, DataType>(v[0], v[1], v[2]);
    Merge3x2<SortRule, DataType>(v[0], v[1], v[2]);
    for (int32_t i = 0, index = 0; index + 2 <= dataLen; index += 2, i++) {
        vst1q_s64(array + index, v[i]);
    }
}
...
static void ALWAYS_INLINE Sort4Rows(int64_t *array, uint64_t *arrayAddr, int32_t dataLen)
{
    if constexpr (std::is_same_v<VType, int64x1_t>) {
        ...
        Sort4<int64x1_t, uint64x1_t, SortRule, DataType>(v0, v1, v2, v3, v0Addr, v1Addr, v2Addr, v3Addr);
        vst1_s64(array, v0);
        vst1_s64(array + 1, v1);
        vst1_s64(array + 2, v2);
        ...
        Sort4<int64x2_t, uint64x2_t, SortRule, DataType>(v[0], v[1], v[2], v[3], vAddr[0], vAddr[1], vAddr[2],
            vAddr[3]);
        Merge4x2<SortRule, DataType>(v[0], v[1], v[2], v[3], vAddr[0], vAddr[1], vAddr[2], vAddr[3]);
        for (i = 0, index = 0; index + 2 <= dataLen; index += 2, i++) {
            vst1q_s64(array + index, v[i]);
            vst1q_u64(arrayAddr + index, vAddr[i]);
        }
...
template <class RawDataType> void SmallCaseSortAsec(int64_t *array, uint64_t *address, int32_t from, int32_t to)
{
    ...
    if constexpr (std::is_same_v<RawDataType, double>) {
        smallCaseSortFuncsAsecDouble[funcId](array + from, address + from, dataLen);
    } else {
...
template <class RawDataType> void SmallCaseSortDesc(int64_t *array, uint64_t *address, int32_t from, int32_t to)
{
    ...
    uint32_t funcId = GenFuncId(dataLen);
    if constexpr (std::is_same_v<RawDataType, double>) {
        smallCaseSortFuncsDescDouble[funcId](array + from, address + from, dataLen);
    } else {
