...
static DataTypesPtr CreateType(DataTypeId dataTypeId)
{
    ...
    switch (dataTypeId) {
        ...
        case OMNI_VARCHAR:
            ...
            break;
        default:
            throw OmniException("Invalid Argument", "Invalid type " + TypeUtil::TypeToString(dataTypeId));
    }
    ...
    const Results &results)
{
    ...
    switch (outType) {
        ...
        case OMNI_VARCHAR:
            ...
            break;
        default:
            throw OmniException("Invalid Argument", "Invalid type " + TypeUtil::TypeToString(outType));
    }
}
...
TEST_P(AggregatorCastTest, verify_cast)
{
    ...
    switch (inType) {
        ...
        case OMNI_VARCHAR:
            ...
            break;
        default:
            throw OmniException("Invalid Argument", "Invalid type " + TypeUtil::TypeToString(inType));
    }
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    [](const testing::TestParamInfo<AggregatorCastTest::ParamType>& info) {
        return TypeUtil::TypeToString(std::get<0>(info.param)) + "_"
            + std::to_string(std::get<1>(info.param)) + "_"
            + TypeUtil::TypeToString(std::get<2>(info.param));
    });
}
