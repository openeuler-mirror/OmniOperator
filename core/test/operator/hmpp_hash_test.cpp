...
TEST(HmppHash, Longhashtest)
{
    ...
    int64_t hashVal;
    int8_t *nullAddr = nullptr;
    LongVector *longVector = new LongVector(allocator, 4);
...
TEST(HmppHash, Doublehashtest)
{
    VectorAllocator *allocator = VectorAllocator::GetGlobalAllocator()->NewChildAllocator("DoubleVector_hmpp");

    DoubleVector *doubleVector = new DoubleVector(allocator, 4);
    double doubleValue = 20.0;
    for (int i = 0; i < 4; i++) {
        doubleVector->SetValue(i, doubleValue);
    }
    doubleVector->SetValueNull(0);
    int64_t hashVal;
    int8_t *nullAddr = nullptr;
...
TEST(HmppHash, Boolhashtest)
{
    ...
    BooleanVector *booleanVector = new BooleanVector(allocator, 4);
