...
//     for (int32_t i = 0; i < pageNum; ++i) {
//         opAddr = executeNativeOmniHashAggregationOperatorLlvm(opAddr, columnTypes1, input[i]->getColumnNumber(), (void**)input[i]->getHeads(), input[i]->getColumnNumber(), input[i]->getPositionCount());
//     }
//     std::vector<Table*> result;
...
//     uint64_t moduleAddr = prepareNativeOmniHashAggregationOperator(groupColsContext, groupTypesContext, aggColsContext, aggTypesContext, aggFunTypeContext, retTypesContext);
