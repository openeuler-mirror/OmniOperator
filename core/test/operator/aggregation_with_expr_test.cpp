...
TEST(HashAggregationWithExprOperatorTest, test_hashagg_partial_flat_output_expr)
{
    ...
    auto *hashAggWithExprOperatorFactory = new HashAggregationWithExprOperatorFactory(groupByKeys, groupByNum, aggAllKeys,
                                                                                      sourceTypes, aggsOutputTypes, aggFuncTypes, maskCols, inputRawWrap, outputPartialWrap, overflowConfig);
    auto *hashAggWithExprOperator =
            dynamic_cast<HashAggregationWithExprOperator *>(CreateTestOperator(hashAggWithExprOperatorFactory));
    ...
    double expData5[] = {34};
    int64_t expData6[] = {8};
    DataTypes expectTypes(std::vector<DataTypePtr>({ LongType(), IntType(), DoubleType(), LongType(), DoubleType(), LongType() }));
    VectorBatch *expectVecorBatch =
...
TEST(HashAggregationWithExprOperatorTest, test_hashagg_final_flat_input_expr)
{
    ...
    DataTypes sourceTypes(std::vector<DataTypePtr>({  LongType(), IntType(), DoubleType(), LongType(), DoubleType(), LongType() }));
    ...
    auto outputPartialWrap = AggregatorUtil::WrapWithVector(false, aggFuncTypes.size());
    auto *hashAggWithExprFinalOperatorFactory = new HashAggregationWithExprOperatorFactory(groupByKeys, groupByNum, aggAllKeys,
                                                                                           sourceTypes, aggsOutputTypes, aggFuncTypes, maskCols, inputRawWrap, outputPartialWrap, overflowConfig);
