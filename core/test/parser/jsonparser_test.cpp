...
map<Operator, string> ArithOps = {{ADD,"ADD"},{SUB,"SUBTRACT"},{MUL,"MULTIPLY"},{DIV,"DIVIDE"},{MOD,"MODULUS"}};
map<Operator, string> CmpOps = {{GT,"GREATER_THAN"},{GTE,"GREATER_THAN_OR_EQUAL"},{LT,"LESS_THAN"},
                                {LTE,"LESS_THAN_OR_EQUAL"},{EQ,"EQUAL"},{NEQ,"NOT_EQUAL"}};
...
string getDec64TestJson (int64_t val, int32_t precision, int32_t scale) {
    ss.str("");
    ss << R"({ "exprType": "LITERAL", "dataType": 6, "isNull": false, "value": )" << val
...
string getDec128TestJson (int64_t val, int32_t precision, int32_t scale) {
    ss.str("");
    ss << R"({ "exprType": "LITERAL", "dataType": 7, "isNull": false, "value": )" << val
        << R"(, "precision": )" << precision << R"(, "scale": )" << scale << R"(})";
    return ss.str();
}
...
string getAndTestJson (const string& left, const string& right) {
    ss.str("");
    ss << R"({"exprType": "BINARY", "returnType": 4, "operator": "AND", "left":)"
...
string getOrTestJson (const string& left, const string& right) {
    ss.str("");
    ss << R"({"exprType": "BINARY", "returnType": 4, "operator": "OR", "left":)"
       << left << R"(, "right":)" << right << R"(})";
    return ss.str();
}
...
string getCoalesceTestJson (int32_t rt, const string& left, const string& right) {
    ss.str("");
    ss << R"({"exprType": "COALESCE", "returnType": )" << rt
        << R"(, "value1": )" << left << R"(, "value2": )" << right  << R"(})";
    return ss.str();
}

string getBetweenTestJson (const string& val, const string& val1, const string& val2) {
    ss.str("");
    ss << R"({"exprType": "BETWEEN", "returnType": 4, "value": )" << val
        << R"(, "lower_bound": )" << val1 << R"(, "upper_bound": )" << val2 << R"(})";
    return ss.str();
}

string getIfTestJson (int32_t rt, const string& val, const string& val1, const string& val2) {
    ss.str("");
    ss << R"({"exprType": "IF", "returnType": )" << rt << R"(, "condition": )" << val
        << R"(, "if_true": )" << val1 << R"(, "if_false": )" << val2 << R"(})";
    return ss.str();
}
...
string getBinaryTestJson (int32_t rt, const string& op, const string& left, const string& right) {
    ss.str("");
    ss << R"({ "exprType": "BINARY", "returnType": )" << rt << R"(, "operator": ")" << op
       << R"(", "left": )" << left << R"(, "right": )" << right << R"(})";
    return ss.str();
}
...
public:
    ...
    template <typename T>
    explicit TestDataExpr(T val): expr(make_unique<DataExpr>(val).release()){
        dataType = expr->dataType;
    }
...
public:
    ...
    bool operator== (const BinaryExpr& rhs) const {
        return (op == rhs.op && dataType == rhs.dataType &&
            left->isEqual(rhs.left) && right->isEqual(rhs.right));
    }
...
public:
    TestIfExpr(TestExpr* cond, TestExpr* tExr, TestExpr* fExp)
            :condition(cond), tExpr(tExr), fExpr(fExp) {
    ...
    bool operator== (const IfExpr& rhs) const {
        return (dataType == rhs.dataType && condition->isEqual(rhs.condition)
...
TEST(JSONParserTest, BinaryExpr_ADD) {
    string unparsedBinaryJson = getBinaryTestJson(INT64D, ArithOps.at(ADD),
                                                  getInt32TestJSON(int32Val),
                                                  getInt64TestJson(int64Val));
    Expr* addExpr = JSONParser::ParseJSON(nlohmann::json::parse(unparsedBinaryJson));
    TestBinaryExpr expectedExpr(ADD,
...
TEST(JSONParserTest, BinaryExpr_EQ) {
    string unparsedBinaryJson = getBinaryTestJson(BOOLD, CmpOps.at(EQ),
                                                  getInt32TestJSON(int32Val),
                                                  getInt64TestJson(int64Val));
    Expr* addExpr = JSONParser::ParseJSON(nlohmann::json::parse(unparsedBinaryJson));
    TestBinaryExpr expectedExpr(EQ,
                                make_unique<TestDataExpr>(int32Val).release(),
                                make_unique<TestDataExpr>(int64Val).release());
    expectedExpr.isEqual(addExpr);
    delete addExpr;
...
TEST(JSONParserTest, BetweenExpr) {
    string unparsedBetweenJson = getBetweenTestJson(getInt32TestJSON(int32Val),
                                                    getInt32TestJSON(int32Val),
                                                    getInt32TestJSON(int32Val));
...
TEST(JSONParserTest, CoalesceExpr) {
    string unparsedCoalesceJson = getCoalesceTestJson(VARCHARD,
                                                      getVarcharTestJson(varcharVal, varcharWidth),
                                                      getVarcharTestJson("", 0));
...
TEST(JSONParserTest, FuncExpr_substr) {
    ...
    vector<TestExpr*> args = { make_unique<TestDataExpr>(make_unique<string>(varcharVal).release()).release(),
                               make_unique<TestDataExpr>(int32Val).release()};
    TestFuncExpr expectedExpr(VARCHARD,
                              "substr",
                              args);
...
TEST(JSONParserTest, InExpr) {
    ...
    vector<TestExpr*> args = {make_unique<TestDataExpr>(int32Val).release(),
                              make_unique<TestDataExpr>(0).release(),
...
TEST(JSONParserTest, IfExpr) {
    string conditionJson = getBinaryTestJson(BOOLD, CmpOps.at(EQ),
                                             getInt32TestJSON(int32Val), getInt64TestJson(int64Val));
    string unparsedIfJson = getIfTestJson(INT32D,
                                          conditionJson,
                                          getInt32TestJSON(int32Val),
                                          getInt32TestJSON(4));
    Expr* ifExpr = JSONParser::ParseJSON(nlohmann::json::parse(unparsedIfJson));
    TestBinaryExpr *condition = new TestBinaryExpr(EQ,
                              make_unique<TestDataExpr>(int32Val).release(),
                              make_unique<TestDataExpr>(int64Val).release());
    TestIfExpr expectedExpr(condition,
                            make_unique<TestDataExpr>(int32Val).release(),
