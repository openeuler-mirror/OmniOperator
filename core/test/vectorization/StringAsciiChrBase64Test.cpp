 ...
 * Description: Unit tests for string functions ascii, chr, char, base64, unbase64, flink_base64
 */
...
// Flink TO_BASE64 semantics: RFC 4648 standard encoding, NO MIME line wrapping (no CRLF).
TEST_F(StringAsciiChrBase64Test, FlinkBase64Basic) {
    std::string b0 = "Man";
    std::string b1 = "hello world";
    std::string b2 = "Spark SQL";
    std::string b3 = "\x01";
    ...
    auto signature = std::make_shared<FunctionSignature>("flink_base64",
    ...
    EXPECT_EQ(std::string(outStrVec->GetValue(2)), "U3BhcmsgU1FM");
    EXPECT_EQ(std::string(outStrVec->GetValue(3)), "AQ==");
    // 57 bytes -> exactly 76 Base64 chars (no CRLF either way; one full MIME line)
    EXPECT_EQ(std::string(outStrVec->GetValue(4)),
        "QUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFB");
    // 58 bytes -> crosses MIME line boundary; flink_base64 emits NO CRLF (single 80-char line).
    // Contrast Spark base64 which inserts "\r\n" before the trailing "QQ==".
    EXPECT_EQ(std::string(outStrVec->GetValue(5)),
        "QUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQQ==");
    // Explicit guard: no CRLF anywhere in the long-output row.
    EXPECT_EQ(std::string(outStrVec->GetValue(5)).find("\r\n"), std::string::npos);

    delete resultVector;
    delete inputVec;
}

// ---- flink_base64: empty input ----
TEST_F(StringAsciiChrBase64Test, FlinkBase64Empty) {
    std::string b0;
    constexpr int rowSize = 1;
    vec::BaseVector* inputVec = VectorHelper::CreateStringVector(rowSize);
    inputVec->SetIsField(true);
...
// ---- flink_base64: binary data with special characters (passed as VARCHAR) ----
TEST_F(StringAsciiChrBase64Test, FlinkBase64BinaryData) {
    std::string b0 = "\xff\xee";
    std::string b1 = std::string("\x00\x01\x02", 3);
    constexpr int rowSize = 2;
    vec::BaseVector* inputVec = VectorHelper::CreateStringVector(rowSize);
...
// ---- flink_base64 + unbase64 roundtrip (long input crossing the 76-char MIME boundary) ----
TEST_F(StringAsciiChrBase64Test, FlinkBase64UnBase64Roundtrip) {
    std::string original = "Flink TO_BASE64: no MIME line wrapping roundtrip test string!!!";
    constexpr int rowSize = 1;
    ...
    auto flinkBase64Sig = std::make_shared<FunctionSignature>("flink_base64",
        std::vector<DataTypeId>{OMNI_VARCHAR}, OMNI_VARCHAR);
    auto flinkBase64Func = VectorFunction::Find(flinkBase64Sig);
    ASSERT_NE(flinkBase64Func, nullptr);
    ...
    ASSERT_NO_THROW(flinkBase64Func->Apply(encodeArgs, varcharType, encodedVector, &ctx1));
    auto* encodedStrVec = dynamic_cast<Vector<LargeStringContainer<std::string_view>>*>(encodedVector);
    ASSERT_NE(encodedStrVec, nullptr);
    // 63 bytes -> 84 Base64 chars, no CRLF (would cross MIME line boundary under Spark base64)
    EXPECT_EQ(std::string(encodedStrVec->GetValue(0)),
        "RmxpbmsgVE9fQkFTRTY0OiBubyBNSU1FIGxpbmUgd3JhcHBpbmcgcm91bmR0cmlwIHRlc3Qgc3RyaW5nISEh");
    EXPECT_EQ(std::string(encodedStrVec->GetValue(0)).find("\r\n"), std::string::npos);
