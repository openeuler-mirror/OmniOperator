...
// Test: Minute from DATE32 - should always return 0
TEST(MinuteTest, Date32Basic) {
    std::cout << "=== Test: Minute from DATE32 - should return 0 ===" << std::endl;
    
    // DATE32 values (days since epoch)
    std::vector<int32_t> dateValues = {0, 100, 1000, 18262};  // Various dates
    std::vector<int32_t> expected = {0, 0, 0, 0};  // All should be 0
    
    BaseVector* inputVec = MinuteFunctionTestHelper::CreateDate32Vector(dateValues);
    BaseVector* resultVec = nullptr;
    MinuteFunctionTestHelper::ExecuteMinute(inputVec, OMNI_DATE32, resultVec);
    MinuteFunctionTestHelper::ValidateResult(resultVec, expected, dateValues.size());
...
// Test: Minute from DATE32 with NULL values
TEST(MinuteTest, Date32WithNull) {
    std::cout << "=== Test: Minute from DATE32 with NULL values ===" << std::endl;
    
    std::vector<int32_t> dateValues = {0, 100, 1000};
    ...
    // First and third should have values (0), second should be NULL
    EXPECT_FALSE(resultVec->IsNull(0)) << "Row 0 should not be NULL";
    EXPECT_TRUE(resultVec->IsNull(1)) << "Row 1 should be NULL";
    EXPECT_FALSE(resultVec->IsNull(2)) << "Row 2 should not be NULL";
    
    // Validate non-null values (all should be 0)
    auto* resultVecTyped = dynamic_cast<Vector<int32_t>*>(resultVec);
    EXPECT_EQ(resultVecTyped->GetValue(0), 0) << "Row 0 minute should be 0";
    EXPECT_EQ(resultVecTyped->GetValue(2), 0) << "Row 2 minute should be 0";
