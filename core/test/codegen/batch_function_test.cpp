...
TEST(BatchFunctionTest, SubstrZh)
{
    ...
    vector<string> expected { str, "时欧基乌斯", "hello! 回复哦", "色的圣诞袜", "", "袜", "", str };
    for (int32_t i = 0; i < rowCnt; i++) {
        string actual(reinterpret_cast<char *>(outResult[i]), outLen[i]);
...
TEST(BatchFunctionTest, SubstrWithStartZh)
{
    ...
    vector<string> expected { str, " hello! 回复哦黑色的and magic粉色的圣诞袜", "圣诞袜", "", "袜", "", str };
    for (int32_t i = 0; i < rowCnt; i++) {
        string actual(reinterpret_cast<char *>(outResult[i]), outLen[i]);
        EXPECT_EQ(actual, expected[i]);
    }
...
TEST(BatchFunctionTest, SubstrWithZhForSpark)
{
    ...
    vector<string> inputStr { 4, str };
    int32_t rowCnt = inputStr.size();
    vector<int32_t> inputLen { strLen, strLen, strLen, strLen };
    vector<int32_t> outLen(inputStr.size());
    vector<uint8_t *> outResult(inputStr.size());
    vector<int32_t> startIndexs { -15, -15, -15, -15 };
    ...
    BatchSubstr(contextPtr, strAddr.data(), inputLen.data(), startIndexs.data(), length.data(), isAnyNull1,
                outResult.data(), outLen.data(), rowCnt);
    vector<string> expected { "", "时", "时欧基乌斯侧后解 ", "时欧基乌斯侧后解 h" };
    for (int32_t i = 0; i < rowCnt; i++) {
        string actual(reinterpret_cast<char *>(outResult[i]), outLen[i]);
        EXPECT_EQ(actual, expected[i]);
    }
...
TEST(BatchFunctionTest, LengthStrZh)
{
    ...
    vector<string> inputStr { str, "解 hello! 回复哦黑色的" };
...
TEST(BatchFunctionTest, LikeStrZh)
{
    ...
    vector<string> inputStr { 4, str };
    vector<int32_t> inputLen { strLen, strLen, strLen, strLen };
    vector<string> patternStr { "^时欧基乌斯侧后解 hello! 回复哦黑色的and magic粉色的圣诞.$",
...
{
    vector<string> inputStr { "时欧基乌斯侧后解 hello! 回复哦黑色的and magic粉色的圣诞袜", "时欧基乌", "时欧基乌",
                              "时欧基乌" };
    vector<string> patternStr { "^时欧基乌斯侧后解 hello! 回复哦黑色的and magic粉色的圣诞.$", "^时欧基乌..$",
                                "^时欧基乌.$", "^时欧基乌.*$" };
    int32_t batch = 4;
...
TEST(BatchFunctionTest, ReplaceStrStrStrZh)
{
    auto context = new ExecutionContext();
    auto contextPtr = reinterpret_cast<int64_t>(context);
    vector<string> str { "apple", "粉色的圣诞袜", "粉色de圣诞袜", "粉色de圣诞袜", "粉色de圣诞袜", "", "粉色de圣诞袜" };
...
TEST(BatchFunctionTest, CastStrWithDiffWidths)
{
    auto context = new ExecutionContext();
    auto contextPtr = reinterpret_cast<int64_t>(context);
    vector<string> srcStr { "cast乌s斯侧后解", "cast乌s斯解", "cast乌s斯侧后解", "cast乌s" };
    vector<int32_t> srcWidth { 18, 18 };
    vector<int32_t> dstWidth { 1024, 7 };
    vector<string> expected { "cast乌s斯侧后解", "cast乌s斯解", "cast乌s斯", "cast乌s" };
    int32_t batch = 2;
    int32_t rowCnt = 2;
    for (int32_t i = 0; i < batch; i++) {
        vector<uint8_t *> output(rowCnt);
        vector<int32_t> outLen(rowCnt);
        vector<int32_t> strLen(rowCnt);
        vector<uint8_t *> srcAddr(rowCnt);
        for (int32_t row = 0; row < rowCnt; row++) {
            int32_t index = i * rowCnt + row;
