...
namespace omniruntime::vec::test {
    ...
    template<typename T>
    void bm_vector_create_with_share_ptr(benchmark::State &state) {
        int vec_size = 10000;
        for (auto _ : state) {
            auto vector = std::make_shared<Vector<T>>(vec_size);
        }
    }

    template<typename T>
    void bm_vector_create(benchmark::State &state) {
        int vec_size = 10000;
        for (auto _ : state) {
    ...
    template<typename T>
    void bm_vector_setvalue(benchmark::State &state) {
        int vec_size = 10000;
        auto vector = std::make_shared<Vector<T>>(vec_size);
        for (auto _ : state) {
            for (int i = 0; i < 1'000'000; i++) {
                T assign(i * 2);
                vector->SetValue(i % vec_size, assign);
            }
        }
    }

    template<typename T>
    void bm_vector_getvalue(benchmark::State &state) {
        int vec_size = 10000;
        auto vector = new Vector<T>(vec_size);
        ...
        for (auto _ : state) {
            for (int i = 0; i < 1'000'000; i++) {
                total += vector->GetValue(i % vec_size);
            }
        }
    ...
    template<typename T>
    void bm_vector_has_no_null(benchmark::State &state) {
        int vec_size = 1'000'001;
        bool hasNull;
        auto vector = std::make_shared<Vector<T>>(vec_size);
        for (auto _ : state) {
    ...
    template<typename T>
    void bm_vector_has_null(benchmark::State &state) {
        int vec_size = 1'000'001;
        bool hasNull;
        auto vector = std::make_shared<Vector<T>>(vec_size);
    ...
    template<typename T>
    void bm_slice_vector_getvalue(benchmark::State &state) {
        ...
        auto vector = parent->Slice(offset, vec_size);
        T total = 0;
        for (auto _ : state) {
            for (int i = 0; i < 1'000'000; i++) {
                total += vector->GetValue(i % vec_size);
    ...
    template<>
    void bm_slice_vector_getvalue<std::string>(benchmark::State &state) {
        int parent_vec_size = 20000;
        int vec_size = 10000;
        int offset = 867;
        ...
        { //scope to ensure deletion of parent
            ...
            vector = parent->Slice(offset, vec_size);
        }
        long total = 0;
        for (auto _ : state) {
            for (int i = 0; i < 1'000'000; i++) {
                total += vector->GetValue(i % vec_size).length();
            }
        }
    ...
    template<>
    void bm_vector_setvalue<std::string>(benchmark::State &state) {
        int vec_size = 10000;
        ...
        for (auto _ : state) {
            auto vector = new Vector<std::string>(vec_size);
            for (int i = 0; i < 1'000'000; i++) {
                std::string value{prefix + std::to_string(i % 100000)};
                vector->SetValue(i % vec_size, value);
            }
            delete vector;
        }
    }
    ...
    template<>
    void bm_vector_getvalue<std::string>(benchmark::State &state) {
        int vec_size = 10000;
        std::string prefix = "hello: ";
        if (state.range(0)) {
            prefix = "hello hello hello hello hello: ";
    ...
    template<typename T>
    static void bm_dictvector_setvalue(benchmark::State &state) {
        int dictionary_size = 100, value_size = 10000;
    ...
    template<typename T>
    static void bm_dictvector_getvalue(benchmark::State &state) {
    ...
    static void bm_dictvector_getvalue_string(benchmark::State &state) {
        ...
        auto container = std::make_shared<DictionaryContainer<std::string_view, LargeStringContainer>>
        (values, value_size, dictionary, dictionary_size, 0);
    ...
    template <typename T>
    void bm_vector_copypositions(benchmark::State &state) {
    ...
    template<>
    void bm_vector_copypositions<std::string>(benchmark::State &state) {
        int originSize = 20000;
        int positionsCnt = 10000;
        int positions[positionsCnt];
        int copySize = state.range(0);
