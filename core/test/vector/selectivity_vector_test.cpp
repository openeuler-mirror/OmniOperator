...
TEST(SelectivityVector, test_simd_set_mask_1)
{
    constexpr int32_t dataSize = 100;
    int64_t data1[dataSize] = {0};
    int64_t data2[dataSize] = {0};
    for (int i = 0; i < dataSize; i++) {
        if (i % 2 == 0) {
            data1[i] = i;
            data2[i] = -i;
    ...
    SelectivityVector selVec(dataSize);
    for (int i = 0; i < dataSize; i += (xsimd::batch<int64_t>::size)) {
        xsimd::batch<int64_t, xsimd::default_arch> a = xsimd::load(data1 + i);
        xsimd::batch<int64_t, xsimd::default_arch> b = xsimd::load(data2 + i);
        xsimd::batch_bool<int64_t, xsimd::default_arch> mask = a < b;
        selVec.SetBitsFromSimdMask(mask, i);
    }
    for (int i = 0; i < dataSize; i++) {
        if (i % 2 == 0) {
            EXPECT_EQ(selVec.IsValid(i), false);
        } else {
            EXPECT_EQ(selVec.IsValid(i), true);
        }
    }
    EXPECT_EQ(selVec.IsAllSelected(), false);
}

TEST(SelectivityVector, test_simd_set_mask_2)
{
    constexpr int32_t dataSize = 100;
    ...
    SelectivityVector selVec(dataSize, false);
    for (int i = 0; i < dataSize; i += (xsimd::batch<int64_t>::size)) {
        xsimd::batch<int64_t, xsimd::default_arch> a = xsimd::load(data1 + i);
        xsimd::batch<int64_t, xsimd::default_arch> b = xsimd::load(data2 + i);
        xsimd::batch_bool<int64_t, xsimd::default_arch> mask = a > b;
    ...
    selVec.UpdateBounds();
    EXPECT_EQ(selVec.IsAllSelected(), true);
}
}