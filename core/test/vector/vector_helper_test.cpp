...
TEST(VectorHelper, setAndGetValue)
{
    ...
    VectorHelper::SetValue(vector6.get(), 5, &value6, OMNI_VARCHAR);
    EXPECT_EQ(vector6->GetValue(5), value6);

    // decimal test
    auto vector7 = std::make_unique<Vector<Decimal128>>(10);
    Decimal128 value7(111, 222);
    VectorHelper::SetValue(vector7.get(), 5, &value7, OMNI_DECIMAL128);
...
TEST(VectorHelper, printVectorValue)
{
    ...
    VectorHelper::SetValue(varcharVec.get(), 5, &value, OMNI_VARCHAR);
    for (int32_t rowIndex = 0; rowIndex < value_size; ++rowIndex) {
        VectorHelper::PrintVectorValue(varcharVec.get(), rowIndex, OMNI_VARCHAR);
    }
}
}
