...
public:
    ...
    void SortMergeJoinTest(benchmark::State& state) {
        ...
        smjOp->ConfigStreamedTblInfo(streamedTblTypes, STREAMED_KEYCOLS[groupKey], STREAMED_OUTPUTCOLS[groupKey], streamedTblTypes.GetSize());

        DataTypes bufferedTblTypes(BUFFER_TYPES[groupKey]);
        smjOp->ConfigBufferedTblInfo(bufferedTblTypes, BUFFER_KEYCOLS[groupKey], BUFFER_OUTPUTCOLS[groupKey], bufferedTblTypes.GetSize());
        
        smjOp->InitScannerAndResultBuilder(nullptr);
        
        auto streamedValues = CreateVectorValues(65538*1000, 2, 1);
        std::string strPrefix = "smj";
        VectorBatch* streamedVecBatch = om_benchmark::CreateVectorBatch(OMNI_FLAT, streamedTblTypes.Get(), strPrefix, streamedValues,65538*1000);

        auto bufferValues = CreateVectorValues(65538*1000, 2, 3);
        VectorBatch* bufferVecBatch = om_benchmark::CreateVectorBatch(OMNI_FLAT, streamedTblTypes.Get(), strPrefix, bufferValues,65538*1000);
